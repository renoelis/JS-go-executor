...
func (be *BufferEnhancer) addBufferPrototypeMethods(runtime *goja.Runtime, prototype *goja.Object) {
	...
	writeFunc := func(call goja.FunctionCall) goja.Value {
		...
		if length < 0 {
			errObj := runtime.NewGoError(fmt.Errorf("The value of \"length\" is out of range. It must be >= 0 && <= %d. Received %d", bufferLength-offset, length))
			errObj.Set("code", runtime.ToValue("ERR_OUT_OF_RANGE"))
			errObj.Set("name", runtime.ToValue("RangeError"))
			panic(errObj)
		}
		...
		if shouldUseFastPath(bufferLength) {
			...
			if bufferBytes != nil && int64(len(bufferBytes)) >= bufferLength {
				...
				if isUTF16 {
					...
					for i := searchStart; i <= len(bufferBytes)-len(searchBytes); i += 2 {
						if bytes.Equal(bufferBytes[i:i+len(searchBytes)], searchBytes) {
							return runtime.ToValue(int64(i))
						}
					}
					return runtime.ToValue(-1)
				} else {
			...
			if callable, ok := goja.AssertFunction(value); ok && callable != nil {
				...
				if obj != nil && !objConversionFailed {
					...
					if isArray {
						...
						length := lengthVal.ToInteger()
						if length > 0 {
							// 🔥 修复：处理不同类型的 TypedArray
							// 需要根据 BYTES_PER_ELEMENT 来确定每个元素的字节数

							// 检测 TypedArray 类型
							bytesPerElement := int64(1) // 默认 1 字节（Uint8Array, Int8Array）
							if bpeVal := obj.Get("BYTES_PER_ELEMENT"); !goja.IsUndefined(bpeVal) {
								bytesPerElement = bpeVal.ToInteger()
							}

							// 🔥 修复：通过 constructor.name 判断 TypedArray 类型
							// 这是最可靠的方法，因为 Float32Array 和 Int32Array 的 BYTES_PER_ELEMENT 都是 4
							isFloatArray := false
							isFloat32 := false
