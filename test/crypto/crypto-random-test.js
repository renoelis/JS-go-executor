...
//
// 测试 14: randomBytes(0) - 应该返回空 Buffer
//
try {
  const buf = crypto.randomBytes(0);
  const passed = Buffer.isBuffer(buf) && buf.length === 0;
  
  addResult(
    '测试 14: randomBytes(0) 返回空 Buffer',
    passed,
    passed ? `正确返回长度为 0 的 Buffer` : `返回了非预期的值`
  );
} catch (error) {
  addResult('测试 14: randomBytes(0) 返回空 Buffer', false, `异常: ${error.message}`);
}

//
// 测试 15: randomBytes(负数) - 应该抛出错误
//
try {
  ...
  addResult(
    '测试 15: randomBytes(负数) 错误处理',
    errorCaught,
    errorCaught ? `正确抛出错误: ${errorMessage}` : `未抛出预期错误`
  );
} catch (error) {
  addResult('测试 15: randomBytes(负数) 错误处理', false, `异常: ${error.message}`);
}

//
// 测试 16: randomBytes() - 超大尺寸限制
//
try {
  let errorCaught = false;
  let success = false;
  ...
  addResult(
    '测试 16: randomBytes() 超大尺寸处理',
    passed,
    message
  );
} catch (error) {
  addResult('测试 16: randomBytes() 超大尺寸处理', false, `异常: ${error.message}`);
}
