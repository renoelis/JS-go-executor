...
console.log('========================================');
console.log('  RSA 综合测试 - Part 5: KeyObject 与高级功能 (Node.js 兼容版)');
console.log('========================================\n');
...
test('1.1 公钥 asymmetricKeyType', () => {
  if (publicKey.asymmetricKeyType !== 'rsa') throw new Error(`应该是 'rsa'，实际是 '${publicKey.asymmetricKeyType}'`);
});

test('1.2 私钥 asymmetricKeyType', () => {
  if (privateKey.asymmetricKeyType !== 'rsa') throw new Error(`应该是 'rsa'，实际是 '${privateKey.asymmetricKeyType}'`);
});

test('1.3 公钥 type 属性', () => {
  if (publicKey.type !== 'public') throw new Error(`应该是 'public'，实际是 '${publicKey.type}'`);
});

test('1.4 私钥 type 属性', () => {
  if (privateKey.type !== 'private') throw new Error(`应该是 'private'，实际是 '${privateKey.type}'`);
});
...
test('8.1 公钥从私钥提取一致性', () => {
  // Node.js 不支持 createPublicKey(privateKeyObject)
  // 需要先导出再导入
  const privPem = privateKey.export({ format: 'pem', type: 'pkcs8' });
  const privKeyObj = crypto.createPrivateKey(privPem);
  
  // 从私钥导出公钥 PEM
  const pubPemFromPriv = crypto.createPublicKey(privKeyObj).export({ format: 'pem', type: 'spki' });
  const pubPemDirect = publicKey.export({ format: 'pem', type: 'spki' });
  
  const normalize = (s) => s.replace(/\s/g, '');
  if (normalize(pubPemFromPriv) !== normalize(pubPemDirect)) {
...
// ============ 总结 ============
console.log('\n========================================');
console.log('  Part 5 测试总结 (Node.js 兼容版)');
console.log('========================================');
