...
test('3.3 PKCS1 - 最大长度 (k-11)', () => {
  const maxLen = 256 - 11;  // 2048位 = 256字节
...
test('3.4 PKCS1 - 超过最大长度 1 字节', () => {
  try {
    const data = Buffer.alloc(256 - 11 + 1, 0xFF);
...
test('4.1 加密全 0 数据', () => {
  ...
  const encrypted = crypto.publicEncrypt(testPubKey, data);
  const decrypted = crypto.privateDecrypt(testPrivKey, encrypted);
  if (!decrypted.equals(data)) throw new Error('解密结果不匹配');
});
...
test('5.2 PSS saltLength = 最大值', () => {
  // 2048位密钥，SHA-256: max = 256 - 32 - 2 = 222
  const maxSalt = 222;
  const sign = crypto.createSign('sha256');
  sign.update('test');
  const signature = sign.sign({
...
test('6.5 HMAC - 长 key (> block size)', () => {
  const longKey = Buffer.alloc(128, 0xFF);  // SHA-256 block size = 64
...
test('7.2 DER 往返转换', () => {
  ...
  if (!der1.equals(der2)) throw new Error('DER 往返不一致');
});
