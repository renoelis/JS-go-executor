...
function roundTrip(inputStr, expected, optionsParse = {}, optionsStringify = {}) {
  ...
  const mark = same ? '✅' : '❌';
  return `${mark} 来回一致性: 期望 "${expected}", 实际 "${reStr}"\n`;
}

// 1. 基础 parse/stringify + round-trip
let result = "\n========== 测试 1: 基础 parse/stringify ==========\n";
const str1 = "foo=bar&baz=qux";
result += checkResult("parse 基础字符串", qs.parse(str1), {"foo":"bar","baz":"qux"});
result += checkString("stringify 基础对象", qs.stringify({ foo: "bar", baz: "qux" }), "foo=bar&baz=qux");
result += roundTrip(str1, "foo=bar&baz=qux");

// 2. 嵌套对象 + round-trip
const str2 = "outer[inner]=value&outer[inn2]=val2";
result += "\n========== 测试 2: 嵌套对象 ==========\n";
result += checkResult("parse 嵌套对象", qs.parse(str2), {"outer":{"inner":"value","inn2":"val2"}});
const obj2 = { outer: { inner: "value", inn2: "val2" } };
result += checkString("stringify 嵌套对象", qs.stringify(obj2), "outer%5Binner%5D=value&outer%5Binn2%5D=val2");
result += roundTrip(str2, "outer%5Binner%5D=value&outer%5Binn2%5D=val2");

// 3. 数组 + round-trip
const str3 = "arr[]=1&arr[]=2&arr[]=3";
result += "\n========== 测试 3: 数组 ==========\n";
result += checkResult("parse 数组", qs.parse(str3), {"arr":["1","2","3"]});
const obj3 = { arr: ["1","2","3"] };
result += checkString("stringify 数组", qs.stringify(obj3), "arr%5B0%5D=1&arr%5B1%5D=2&arr%5B2%5D=3");
result += roundTrip(str3, "arr%5B0%5D=1&arr%5B1%5D=2&arr%5B2%5D=3");
...
result += "\n========== 测试 4: stringify 含选项 ==========\n";
result += checkString("stringify 使用选项", str4, "a=x;d=m;d=n");
result += checkResult("parse 后对象", qs.parse(str4), {"a":"x;d=m;d=n"});
const re4 = qs.stringify(qs.parse(str4), opts4);
result += checkString("再序列化一致性", re4, "a=x%3Bd%3Dm%3Bd%3Dn");
result += roundTrip(str4, "a=x%3Bd%3Dm%3Bd%3Dn", {}, {});
...
result += checkResult("allowDots:true", qs.parse(str5a, { allowDots:true }), {"a":{"b":"c","d":"e"}});
result += roundTrip(str5a, "a%5Bb%5D=c&a%5Bd%5D=e", { allowDots:true }, {});

result += "\n5.2 depth 选项:";
const str5b = "a[b][c][d][e]=v";
result += checkResult("depth=2", qs.parse(str5b, { depth:2 }), {"a":{"b":{"c":{"[d][e]":"v"}}}});
result += roundTrip(str5b, "a%5Bb%5D%5Bc%5D%5B%5Bd%5D%5Be%5D%5D=v", { depth:2 }, {});

result += "\n5.3 comma 选项:";
const str5c = "x=1,2,3";
result += checkResult("comma:true", qs.parse(str5c, { comma:true }), {"x":["1","2","3"]});
result += roundTrip(str5c, "x%5B0%5D=1&x%5B1%5D=2&x%5B2%5D=3", { comma:true }, {});

result += "\n5.4 delimiter 选项:";
const str5d = "p=1;q=2,r=3";
result += checkResult("delimiter=/[;,]/", qs.parse(str5d, { delimiter:/[;,]/ }), {"p":"1","q":"2","r":"3"});
result += roundTrip(str5d, "p=1&q=2&r=3", { delimiter:/[;,]/ }, {});
...
result += checkResult("默认解析 50 个参数", parsedKeys, 50, false);
...
try {
  ...
  result += "❌ 未抛异常（这里不报错也正常，qs 允许 null）\n";
} catch(e) {
  result += `✅ parse(null) 抛异常: ${e.message}\n`;
}

const emptyStr = "";
result += checkResult("parse 空字符串", qs.parse(emptyStr, { plainObjects:true }), {}, false);
result += checkString("stringify 空对象", qs.stringify({}), "");
result += roundTrip("", "", {}, {});  // test empty string -> object -> string
...
for (const test of testInputs) {
  result += roundTrip(test.input, test.expected);
}
