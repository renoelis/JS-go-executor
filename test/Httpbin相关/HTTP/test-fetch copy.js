...
async function testTimeoutAndSlow() {
  ...
  await test('GET /slow-bytes/1000?delay=50 (慢速传输)', async () => {
    const start = Date.now();
    const res = await fetch(`${BASE_URL}/slow-bytes/1000?delay=50`);
    if (res.status !== 200) throw new Error('状态码错误');
    const buffer = await res.arrayBuffer(); // ✅ 先读取完整 body
    const elapsed = Date.now() - start; // ✅ 然后测量总时间（包含 body 下载）
    if (buffer.byteLength !== 1000) throw new Error(`字节数不匹配: ${buffer.byteLength}`);
    if (elapsed < 400) throw new Error(`延迟不足: ${elapsed}ms (预期 >400ms)`);
    log(`  慢速传输耗时: ${elapsed}ms (分10块传输)`, 'yellow');
  });
}
