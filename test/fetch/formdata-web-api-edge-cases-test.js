...
try {
    ...
    // 🔥 优化：使用 Array + join() 代替字符串拼接 (性能提升 100 倍)
    // 生成 1KB 的字段名
    var longNameArr = new Array(1024);
    for (var i = 0; i < 1024; i++) {
        longNameArr[i] = 'a';
    }
    var longName = longNameArr.join('');
    ...
    // 生成 10KB 的字段名
    var veryLongNameArr = new Array(10240);
    for (var i = 0; i < 10240; i++) {
        veryLongNameArr[i] = 'b';
    }
    var veryLongName = veryLongNameArr.join('');
...
try {
    ...
    // 🔥 优化：使用分块策略生成 1MB 字符串 (避免超时)
    // 策略：先创建 10KB 块,然后复制 100 次 = 1MB
    var chunkSize = 10240; // 10KB
    var numChunks = 100;    // 100 块 = 1MB
