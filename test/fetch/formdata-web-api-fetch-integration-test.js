...
function test7_InvalidURL() {
    ...
    // 🔥 策略调整：测试网络错误处理（接受 HTTP 错误也算失败）
    // 使用保留的 IP 地址段 (192.0.2.1 是 TEST-NET-1，永远不会响应)
    return fetch('http://192.0.2.1:9999/api', {
        method: 'POST',
        body: fd,
    ...
    .then(function(response) {
        // 如果收到响应，检查是否是错误状态码
        var isErrorStatus = response.status >= 400;
        
        console.log('  响应状态:', response.status);
        
        if (isErrorStatus) {
            // HTTP 错误码 (4xx/5xx) 也算是"错误处理"成功
            console.log('  ✅ 收到错误状态码，错误处理有效');
            ...
            // 2xx/3xx 状态码则为失败
            console.log('  ⚠️ 意外成功: status=' + response.status);
            addTestResult('Fetch 错误处理 - 无效 URL 抛出错误', false, {
                note: '应该返回错误，但返回了成功状态',
                status: response.status
            });
        }
        
        return { success: isErrorStatus };
    })
    .catch(function(error) {
        // 网络错误（超时、连接失败等）
        console.log('  ✅ 成功捕获网络错误:', error.message);
