...
function testOptimizations() {
  ...
  // 测试 4: Blob.prototype 上的方法是否可枚举（与 Node.js/浏览器一致）
  results.methodsEnumerable = (function() {
    try {
      const blob = new Blob([]);
      ...
      // ✅ 所有方法都应该可枚举（与 Node.js/浏览器一致）
      const allEnumerable = methods.every(m => enumResults[m] === true);
      
      return {
        ok: allEnumerable,
        details: enumResults,
        enumerableProps,
        expected: 'all true (enumerable, 与 Node.js/浏览器一致)'
      };
    } catch (e) {
      return { ok: false, error: e.message };
    }
  ...
  // 测试 5: constructor 属性是否不可枚举（与 Node.js/浏览器一致）
  results.constructorNonEnumerable = (function() {
    try {
      ...
      return {
        ...
        note: '与 Node.js/浏览器一致：constructor 不可枚举'
      };
    } catch (e) {
      return { ok: false, error: e.message };
    }
