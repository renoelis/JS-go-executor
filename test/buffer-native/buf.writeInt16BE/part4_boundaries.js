...
test('BE 写入后用 readInt16BE 读取验证', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16BE(12345, 0);
  return buf.readInt16BE(0) === 12345;
});

test('BE 写入负数后用 readInt16BE 读取验证', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16BE(-12345, 0);
  return buf.readInt16BE(0) === -12345;
});
...
test('边界值 0x7FFF (32767)', () => {
  ...
  return buf[0] === 0x7F && buf[1] === 0xFF && buf.readInt16BE(0) === 32767;
});

test('边界值 -0x8000 (-32768)', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16BE(-0x8000, 0);
  return buf[0] === 0x80 && buf[1] === 0x00 && buf.readInt16BE(0) === -32768;
});
...
test('在 buffer 末尾边界写入', () => {
  ...
  buf.writeInt16BE(999, 8); // 最后两个字节
  return buf.readInt16BE(8) === 999;
});
...
test('重叠写入测试', () => {
  ...
  // offset 1-2: 12 34
  // 结果: FF 12 34 00
  return buf[0] === 0xFF && buf[1] === 0x12 && buf[2] === 0x34;
});
...
test('写入交替位模式 0xAAAA 会抛出错误', () => {
  try {
    const buf = Buffer.alloc(4);
    buf.writeInt16BE(0xAAAA, 0); // 超出范围
    return false;
  } catch (e) {
    return e.message.includes('out of range');
  }
