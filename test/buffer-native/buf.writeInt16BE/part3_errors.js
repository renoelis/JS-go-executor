...
test('offset + 2 超出 buffer 长度抛出错误', () => {
  try {
    const buf = Buffer.alloc(4);
    buf.writeInt16BE(100, 3); // 需要写入到 offset 3 和 4，但 buffer 只有 4 字节
    return false;
  } catch (e) {
    return e.message.includes('out of range') || e.message.includes('beyond');
  }
...
test('值为 null 会转为 0', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16BE(null, 0);
  return buf[0] === 0x00 && buf[1] === 0x00;
});

test('值为 undefined 会转为 0', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16BE(undefined, 0);
  return buf[0] === 0x00 && buf[1] === 0x00;
});
...
test('offset 为 undefined 会转为 0', () => {
  const buf = Buffer.alloc(4);
  const result = buf.writeInt16BE(100, undefined);
  return result === 2 && buf[0] === 0x00 && buf[1] === 0x64;
});
