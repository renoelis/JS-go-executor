// buf.writeInt16BE() - 第4轮补充：组合场景和交叉测试
...
test('write 后 read 往返测试 - 不同offset', () => {
  ...
  for (let {val, offset} of testData) {
    ...
    if (buf.readInt16BE(offset) !== val) return false;
  }
  return true;
});
...
test('连续写入填满整个 buffer', () => {
  ...
  buf.writeInt16BE(0x0708, 6);
  buf.writeInt16BE(0x090A, 8);
  return buf[0] === 0x01 && buf[1] === 0x02 &&
         buf[2] === 0x03 && buf[3] === 0x04 &&
         buf[4] === 0x05 && buf[5] === 0x06 &&
         buf[6] === 0x07 && buf[7] === 0x08 &&
         buf[8] === 0x09 && buf[9] === 0x0A;
});
...
test('正负值交替写入', () => {
  ...
  buf.writeInt16BE(200, 4);
  buf.writeInt16BE(-200, 6);
  buf.writeInt16BE(300, 8);
  return buf.readInt16BE(0) === 100 &&
         buf.readInt16BE(2) === -100 &&
         buf.readInt16BE(4) === 200 &&
         buf.readInt16BE(6) === -200 &&
         buf.readInt16BE(8) === 300;
});
...
test('写入递增序列', () => {
  ...
  for (let i = 0; i < 10; i++) {
    if (buf.readInt16BE(i * 2) !== i * 100) return false;
  }
  return true;
});
...
test('写入递减序列', () => {
  const buf = Buffer.alloc(20);
  for (let i = 0; i < 10; i++) {
    buf.writeInt16BE(1000 - i * 100, i * 2);
  }
  for (let i = 0; i < 10; i++) {
    if (buf.readInt16BE(i * 2) !== 1000 - i * 100) return false;
  }
  return true;
});

// 与其他位宽的 write 方法配合
test('writeInt16BE 和 writeInt8 交织', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt8(0x11, 0);
  buf.writeInt16BE(0x2233, 1);
  buf.writeInt8(0x44, 3);
  buf.writeInt16BE(0x5566, 4);
  return buf[0] === 0x11 &&
         buf[1] === 0x22 && buf[2] === 0x33 &&
         buf[3] === 0x44 &&
         buf[4] === 0x55 && buf[5] === 0x66;
});

test('writeInt16BE 和 writeInt32BE 交织', () => {
  const buf = Buffer.alloc(10);
...
test('在已有数据上部分更新', () => {
  ...
  buf.writeInt16BE(-21846, 2); // 0xAAAA 作为有符号数
  return buf[0] === 0x11 && buf[1] === 0x22 &&
...
test('同一位置多次写入最后一次生效', () => {
  ...
  buf.writeInt16BE(300, 0);
  buf.writeInt16BE(400, 0);
  return buf.readInt16BE(0) === 400;
});
