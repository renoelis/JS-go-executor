// buf.writeUInt32LE() - Error Handling and Boundary Tests
...
test('偏移量过大', () => {
  try {
    const buf = Buffer.allocUnsafe(4);
    buf.writeUInt32LE(0x12345678, 100);
    return false;
  } catch (e) {
    return e.message.includes('"offset" is out of range');
  }
...
test('this 不是 Buffer 实例', () => {
  try {
    const notBuffer = {};
    Buffer.prototype.writeUInt32LE.call(notBuffer, 123, 0);
    return false;
  } catch (e) {
    return e.message.includes('"offset" is out of range');
  }
...
test('NaN 作为数值', () => {
  try {
    const buf = Buffer.allocUnsafe(4);
    buf.writeUInt32LE(NaN, 0);
    return buf.readUInt32LE(0) === 0; // NaN 转换为 0
  } catch (e) {
    return false;
  }
});
...
test('超出 32 位范围的数值应该抛出异常', () => {
  try {
    const buf = Buffer.allocUnsafe(4);
    buf.writeUInt32LE(0x1FFFFFFFF, 0);
    return false; // 应该抛出异常
  } catch (e) {
    return e.message.includes('"value" is out of range');
  }
...
test('偏移量为小数应该抛出异常', () => {
  try {
    const buf = Buffer.allocUnsafe(8);
    buf.writeUInt32LE(0x12345678, 2.9);
    return false;
  } catch (e) {
