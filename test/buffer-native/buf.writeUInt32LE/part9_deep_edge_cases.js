// buf.writeUInt32LE() - Deep Edge Cases Tests
...
test('极限大缓冲区写入', () => {
  ...
  buf.writeUInt32LE(0x12345678, offset);

  return buf.readUInt32LE(offset) === 0x12345678;
});
...
test('缓冲区末尾极限写入', () => {
  ...
  return buf.readUInt32LE(offset) === 0xFFFFFFFF;
});
...
test('跨页边界写入', () => {
  ...
  try {
    buf.writeUInt32LE(0x11111111, pageSize - 2); // 这个位置是允许的
    // 验证写入成功
    const readValue = buf.readUInt32LE(pageSize - 2);
  ...
  // 测试有效的写入
  buf.writeUInt32LE(0x22222222, pageSize);     // 正好在第二页开始
  return buf.readUInt32LE(pageSize) === 0x22222222;
});
...
test('数值转换的极限情况', () => {
  ...
  // 测试各种极限数值
  buf.writeUInt32LE(0.0000000001, 0);    // 极小的正数
  buf.writeUInt32LE(0.9999999999, 4);    // 接近1的数
  buf.writeUInt32LE(1.0000000001, 8);    // 刚刚超过1
  buf.writeUInt32LE(4294967294.9, 12);   // 接近最大值

  // 测试超过最大值的数值 - 应该抛出异常
  try {
    buf.writeUInt32LE(4294967295.1, 16);   // 超过最大值
    return false; // 应该抛出异常
  } catch (e) {
    // 预期会失败，数值超出范围
  ...
  return buf.readUInt32LE(0) === 0 &&
         ...
         buf.readUInt32LE(12) === 4294967294;
});
...
test('字符串解析的边界情况', () => {
  ...
  // 测试各种字符串格式
  buf.writeUInt32LE('', 0);           // 空字符串
  buf.writeUInt32LE('0', 4);          // 字符串0
  buf.writeUInt32LE(' 123 ', 8);      // 带空格的字符串
  buf.writeUInt32LE('123abc', 12);    // 混合字符串
  buf.writeUInt32LE('0x', 16);        // 不完整十六进制

  // 注意：Node.js中'0xGHI'实际上会被解析为0，而不是抛出异常
  // 因为JavaScript的parseInt('0xGHI')返回NaN，然后被转换为0
  buf.writeUInt32LE('0xGHI', 20);     // 非法十六进制 - 实际被解析为0

  // 注意：空字符串和无效十六进制会被解析为0
  // '123abc'也会被解析为0，因为parseInt('123abc')返回123，但超出范围检查时会失败
  return buf.readUInt32LE(0) === 0 &&
         buf.readUInt32LE(4) === 0 &&
         buf.readUInt32LE(8) === 123 &&
         buf.readUInt32LE(12) === 0 &&  // '123abc' 被解析为0
         buf.readUInt32LE(16) === 0 &&
         buf.readUInt32LE(20) === 0;
});
...
test('偏移量计算的复杂情况', () => {
  ...
  try {
    buf.writeUInt32LE(0x87654321, 4.0000000001); // 极接近4的浮点数 - 应该失败
    return false;
  } catch (e) {
    // 预期会失败，浮点数偏移量不被允许
  ...
  try {
    buf.writeUInt32LE(0x11111111, 4.9999999999); // 极接近5的浮点数 - 应该失败
    return false;
  } catch (e) {
    // 预期会失败，浮点数偏移量不被允许
    if (!e.message.includes('"offset" is out of range')) return false;
...
test('极端温度的数值稳定性', () => {
  ...
  for (let i = 0; i < patterns.length; i++) {
    buf.writeUInt32LE(patterns[i], i * 4);
  }

  // 验证所有模式都能正确写入和读取
  return patterns.every((pattern, i) => buf.readUInt32LE(i * 4) === pattern);
});
...
test('并发写入的内存一致性', () => {
  ...
  positions.forEach((pos, i) => {
    buf.writeUInt32LE(i, pos);
  });

  // 验证所有写入的值都正确
  return positions.every((pos, i) => buf.readUInt32LE(pos) === i);
});
...
test('极端系统压力下的稳定性', () => {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    try {
      ...
      if (buf.readUInt32LE(offset) === value) {
        successCount++;
      }
    } catch (e) {
      // 边界情况，允许失败
...
test('数值表示的完整性验证', () => {
  ...
  return testValues.every(value => {
    ...
    return buf.readUInt32LE(0) === value;
  });
});
...
test('字节序转换的数学正确性', () => {
  ...
  // 手动验证小端字节序（LE）
  const manual = buf[0] | (buf[1] << 8) | (buf[2] << 16) | (buf[3] << 24);
