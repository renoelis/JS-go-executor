// buf.writeUInt32LE() - Input Types Tests
...
test('正整数', () => {
  ...
  return buf.readUInt32LE(0) === 12345;
});
...
test('浮点数转换为整数', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32LE(123.789, 0);
  return buf.readUInt32LE(0) === 123;
});

test('科学计数法', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32LE(1.5e4, 0);
  return buf.readUInt32LE(0) === 15000;
});

test('十六进制字符串', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32LE(0xFF, 0);
  return buf.readUInt32LE(0) === 255;
});

test('二进制字符串', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32LE(0b1010, 0);
  return buf.readUInt32LE(0) === 10;
});

test('八进制字符串', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32LE(0o777, 0);
  return buf.readUInt32LE(0) === 511;
});
...
test('超出32位范围应该抛出异常', () => {
  try {
    const buf = Buffer.allocUnsafe(4);
    buf.writeUInt32LE(Number.MAX_SAFE_INTEGER, 0);
    return false; // 应该抛出异常
  } catch (e) {
...
test('布尔值 true 作为数值', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32LE(true, 0);
  return buf.readUInt32LE(0) === 1;
});

test('布尔值 false 作为数值', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32LE(false, 0);
  return buf.readUInt32LE(0) === 0;
});
...
test('undefined 作为偏移量应该正常工作', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32LE(12345, undefined);
  return buf.readUInt32LE(0) === 12345;
});
...
test('大数值 0x7FFFFFFF', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32LE(0x7FFFFFFF, 0);
