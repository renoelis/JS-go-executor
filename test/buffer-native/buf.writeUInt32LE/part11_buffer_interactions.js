// buf.writeUInt32LE() - Buffer Method Interactions Tests
...
test('写入后 compare 比较', () => {
  ...
  buf1.writeUInt32LE(0x12345678, 0);
  buf2.writeUInt32LE(0x12345679, 0);
...
test('写入后 indexOf 搜索', () => {
  ...
  // 小端字节序：0x12345678 -> [0x78, 0x56, 0x34, 0x12]
  const searchBuf = Buffer.from([0x78, 0x56]);
...
test('写入后 includes 搜索', () => {
  ...
  // 小端字节序：0x12345678 -> [0x78, 0x56, 0x34, 0x12]
  const searchBuf = Buffer.from([0x56, 0x34]);
...
test('写入后 toString 转换', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32LE(0x6C6C6548, 0); // "Hell" in ASCII (little endian)
...
test('连续写入操作', () => {
  ...
  buf.writeUInt32LE(0x12345678, 0);
  buf.writeUInt32LE(0x9ABCDEF0, 4);
  buf.writeUInt32LE(0x11111111, 8);

  return buf.readUInt32LE(0) === 0x12345678 &&
         buf.readUInt32LE(4) === 0x9ABCDEF0 &&
         buf.readUInt32LE(8) === 0x11111111;
});
