// buf.writeUInt32LE() - Basic Functionality Tests
...
test('基本写入功能 - 小数值', () => {
  ...
  if (buf[1] !== 0x56) return false;
  if (buf[2] !== 0x34) return false;
  if (buf[3] !== 0x12) return false;
  return true;
});

test('基本写入功能 - 最大安全整数', () => {
  const buf = Buffer.allocUnsafe(4);
  const result = buf.writeUInt32LE(0xFFFFFFFF, 0);
  if (result !== 4) return false;
...
test('基本写入功能 - 最小值 0', () => {
  const buf = Buffer.allocUnsafe(4);
  const result = buf.writeUInt32LE(0, 0);
  if (result !== 4) return false;
...
test('偏移量写入 - 从中间位置', () => {
  ...
  buf.fill(0);
  const result = buf.writeUInt32LE(0x12345678, 2);
  if (result !== 6) return false;
  if (buf[2] !== 0x78) return false;
  if (buf[3] !== 0x56) return false;
  if (buf[4] !== 0x34) return false;
  if (buf[5] !== 0x12) return false;
  return true;
});

test('返回值测试', () => {
  const buf = Buffer.allocUnsafe(10);
  const result = buf.writeUInt32LE(0x12345678, 3);
...
test('多字节序验证', () => {
  ...
  if (buf[2] !== 0x34) return false;
  if (buf[3] !== 0x12) return false;
  if (buf[4] !== 0xF0) return false;
  if (buf[5] !== 0xDE) return false;
  if (buf[6] !== 0xBC) return false;
  if (buf[7] !== 0x9A) return false;
  return true;
});
...
test('覆盖写入测试', () => {
  ...
  buf.writeUInt32LE(0x22222222, 0);
