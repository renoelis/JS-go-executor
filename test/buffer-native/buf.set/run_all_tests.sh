...
TEST_DIR="$BASE_DIR/test/buffer-native/buf.set"
...
for test_file in "${tests[@]}"; do
  ...
  if [ "$test_failed" -eq 0 ]; then
    ...
    echo ""
    echo "失败的测试:"
    echo "$RESPONSE" | jq -r '.result.tests[] | select(.status == "❌") | "  - " + .name + (if .error then " (" + .error + ")" else "" end)'
  fi
...
if [ "$total_tests" -gt 0 ]; then
  ...
  if [ "$total_failed" -eq 0 ]; then
    echo ""
    echo "🎉 所有测试通过！buf.set API 与 Node.js v25.0.0 完全兼容！"
    exit 0
  else
    echo ""
