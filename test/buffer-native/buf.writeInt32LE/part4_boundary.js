...
test('边界值：127 (0x7F)', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeInt32LE(127, 0);
  return buf[0] === 0x7F;
});

test('边界值：128 (0x80)', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeInt32LE(128, 0);
  return buf[0] === 0x80;
});
...
test('边界值：2^31 - 1 (最大正数)', () => {
  ...
  return buf[0] === 0xFF && buf[1] === 0xFF && buf[2] === 0xFF && buf[3] === 0x7F;
});
...
test('偏移量边界：offset = length - 4（恰好容纳）', () => {
  ...
  const result = buf.writeInt32LE(123, 4);
  return result === 8;
});
...
test('Buffer 长度：很大的 Buffer', () => {
  ...
  const result = buf.writeInt32LE(0x12345678, 512);
  return result === 516 && buf[512] === 0x78;
});
...
test('补码：-2 的表示', () => {
  ...
  return buf[0] === 0xFE && buf[1] === 0xFF && buf[2] === 0xFF && buf[3] === 0xFF;
});

test('补码：-256 的表示', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeInt32LE(-256, 0);
  return buf[0] === 0x00 && buf[1] === 0xFF && buf[2] === 0xFF && buf[3] === 0xFF;
});
