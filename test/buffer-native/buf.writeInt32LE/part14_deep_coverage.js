...
test('传入 3 个参数（第3个参数被忽略）', () => {
  const buf = Buffer.alloc(8);
  const ret = buf.writeInt32LE(0x12345678, 0, 'ignored');
  return ret === 4 && buf[0] === 0x78 && buf[3] === 0x12;
});

test('传入 5 个参数（额外参数被忽略）', () => {
  const buf = Buffer.alloc(8);
  const ret = buf.writeInt32LE(0x12345678, 0, 'a', 'b', 'c');
  return ret === 4;
});
...
test('offset 为 Number.MIN_SAFE_INTEGER 应抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.writeInt32LE(100, Number.MIN_SAFE_INTEGER);
    return false;
  } catch (e) {
...
test('Number.EPSILON 转换为 0', () => {
  const buf = Buffer.alloc(8);
  buf.writeInt32LE(Number.EPSILON, 0);
  return buf.readInt32LE(0) === 0;
});

test('非常小的正数转换为 0', () => {
  const buf = Buffer.alloc(8);
  buf.writeInt32LE(0.000001, 0);
  return buf.readInt32LE(0) === 0;
});
...
test('2^31 超出范围应抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.writeInt32LE(Math.pow(2, 31), 0);
    return false;
  } catch (e) {
...
test('2^32 超出范围应抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.writeInt32LE(Math.pow(2, 32), 0);
    return false;
  } catch (e) {
    return e.name === 'RangeError' || e.message.includes('out of range');
  }
...
test('2^32 - 1 超出范围应抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.writeInt32LE(Math.pow(2, 32) - 1, 0);
    return false;
  } catch (e) {
    return e.name === 'RangeError' || e.message.includes('out of range');
  }
...
test('连续写入多个值', () => {
  ...
  return buf.readInt32LE(0) === 0x11111111 && 
         buf.readInt32LE(4) === 0x22222222 &&
         buf.readInt32LE(8) === 0x33333333 &&
         buf.readInt32LE(12) === 0x44444444;
});
...
test('覆盖写入', () => {
  ...
  return buf.readInt32LE(0) === 0x22222222;
});
...
test('Uint8Array 视图能看到写入的值', () => {
  ...
  buf.writeInt32LE(0x12345678, 0);
  return view[0] === 0x78 && view[1] === 0x56 && view[2] === 0x34 && view[3] === 0x12;
});
...
test('返回值应该是 offset + 4', () => {
  const buf = Buffer.alloc(8);
  const ret0 = buf.writeInt32LE(123, 0);
  const ret2 = buf.writeInt32LE(456, 2);
  const ret4 = buf.writeInt32LE(789, 4);
...
test('返回值可以作为下次调用的 offset', () => {
  const buf = Buffer.alloc(16);
  let offset = 0;
  offset = buf.writeInt32LE(1, offset);
  offset = buf.writeInt32LE(2, offset);
