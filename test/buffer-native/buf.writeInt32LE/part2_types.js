...
test('整数：正整数', () => {
  ...
  const result = buf.writeInt32LE(42, 0);
  return result === 4 && buf[0] === 42;
});
...
test('浮点数：截断为整数', () => {
  const buf = Buffer.allocUnsafe(4);
  const result = buf.writeInt32LE(123.456, 0);
  return result === 4;
});

test('浮点数：负浮点数截断', () => {
  const buf = Buffer.allocUnsafe(4);
  const result = buf.writeInt32LE(-123.789, 0);
  return result === 4;
});
...
test('布尔值：true 转为 1', () => {
  const buf = Buffer.allocUnsafe(4);
  const result = buf.writeInt32LE(true, 0);
  return result === 4 && buf[0] === 1;
});

test('布尔值：false 转为 0', () => {
  const buf = Buffer.allocUnsafe(4);
  const result = buf.writeInt32LE(false, 0);
  return result === 4 && buf[0] === 0;
});

test('null：转为 0', () => {
  const buf = Buffer.allocUnsafe(4);
  const result = buf.writeInt32LE(null, 0);
  return result === 4 && buf[0] === 0;
});

test('undefined：转为 0', () => {
  const buf = Buffer.allocUnsafe(4);
  const result = buf.writeInt32LE(undefined, 0);
  return result === 4;
});

test('NaN：转为 0', () => {
  const buf = Buffer.allocUnsafe(4);
  const result = buf.writeInt32LE(NaN, 0);
  return result === 4 && buf[0] === 0;
});
...
test('偏移量：undefined 转为 0', () => {
  const buf = Buffer.allocUnsafe(4);
  const result = buf.writeInt32LE(123, undefined);
  return result === 4;
});
