// buf.writeUInt32BE() - Extreme Boundary Tests
...
test('4294967295 边界值', () => {
  ...
  buf.writeUInt32BE(4294967295, 0); // 2^32 - 1
...
test('4294967296 超出边界', () => {
  try {
    const buf = Buffer.allocUnsafe(4);
    buf.writeUInt32BE(4294967296, 0); // 2^32
    return false;
  } catch (e) {
...
test('2147483647 有符号边界', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32BE(2147483647, 0); // 2^31 - 1
  return buf.readUInt32BE(0) === 2147483647;
});

test('2147483648 有符号边界+1', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32BE(2147483648, 0); // 2^31
...
test('4294967295.1 浮点边界', () => {
  try {
    const buf = Buffer.allocUnsafe(4);
    buf.writeUInt32BE(4294967295.1, 0);
    return false;
  } catch (e) {
    return e.message.includes('out of range');
  }
...
test('4294967294.9 浮点边界', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32BE(4294967294.9, 0);
  return buf.readUInt32BE(0) === 4294967294;
});

test('0.9999999999999999 接近1', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32BE(0.9999999999999999, 0);
  return buf.readUInt32BE(0) === 0; // 应该截断为0
});

test('1.0000000000000002 略大于1', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32BE(1.0000000000000002, 0);
  return buf.readUInt32BE(0) === 1; // 应该截断为1
});
...
test('1e10 科学计数法', () => {
  try {
    const buf = Buffer.allocUnsafe(4);
    buf.writeUInt32BE(1e10, 0); // 10,000,000,000 > 2^32
    return false;
  } catch (e) {
    return e.message.includes('out of range');
  }
...
test('4.294967295e9 科学计数法边界', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32BE(4.294967295e9, 0);
  return buf.readUInt32BE(0) === 4294967295;
});

test('1e-10 极小科学计数法', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32BE(1e-10, 0);
...
test('字符串 "4294967296" 超出范围', () => {
  try {
    const buf = Buffer.allocUnsafe(4);
    buf.writeUInt32BE('4294967296', 0);
    return false;
  } catch (e) {
    return e.message.includes('out of range');
  }
...
test('字符串 "0x100000000" 十六进制超出', () => {
  try {
    const buf = Buffer.allocUnsafe(4);
    buf.writeUInt32BE('0x100000000', 0);
    return false;
  } catch (e) {
    return e.message.includes('out of range');
  }
...
test('偏移量 2^53', () => {
  try {
    const buf = Buffer.allocUnsafe(4);
    buf.writeUInt32BE(123, Math.pow(2, 53));
    return false;
  } catch (e) {
    return e.message.includes('out of range') || e.message.includes('must be an integer');
  }
...
test('偏移量 2^32', () => {
  try {
    const buf = Buffer.allocUnsafe(4);
    buf.writeUInt32BE(123, Math.pow(2, 32));
    return false;
  } catch (e) {
    return e.message.includes('out of range');
  }
...
test('最大可能的缓冲区长度', () => {
  try {
    ...
    buf.writeUInt32BE(0x12345678, buf.length - 4);
    return buf.readUInt32BE(buf.length - 4) === 0x12345678;
  } catch (e) {
    // 如果无法创建大缓冲区，这是可以接受的
    return true;
  }
...
test('交替位模式 0xAAAAAAAA', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32BE(0xAAAAAAAA, 0);
...
test('交替位模式 0x55555555', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeUInt32BE(0x55555555, 0);
...
test('3字节对齐写入', () => {
  ...
  return buf.readUInt32BE(3) === 0x12345678;
});
...
test('部分覆盖边界写入', () => {
  ...
  buf.writeUInt32BE(0x9ABCDEF0, 2); // 覆盖后两个字节和前两个字节
...
test('偏移量 4.0 精确浮点数', () => {
  const buf = Buffer.allocUnsafe(8);
  buf.writeUInt32BE(0x12345678, 4.0);
...
test('偏移量 4.000000000000001 极小浮点差', () => {
  try {
    const buf = Buffer.allocUnsafe(8);
    buf.writeUInt32BE(0x12345678, 4.000000000000001);
    return false;
  } catch (e) {
