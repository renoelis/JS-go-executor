// buf.writeInt32BE() - 深度类型转换和边缘情况测试
...
test('value 的 Getter valueOf', () => {
  ...
  buf.writeInt32BE(obj, 0);
  return called && buf.readInt32BE(0) === 123;
});
...
test('十六进制字符串', () => {
  const buf = Buffer.alloc(8);
  buf.writeInt32BE('0x10', 0);
  return buf.readInt32BE(0) === 16;
});
