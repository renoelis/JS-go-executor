// buf.writeInt32BE() - 深度补充：跨类型互操作细节
...
test('DataView：Buffer 写入 DataView 读取', () => {
  ...
  return view.getInt32(0, false) === 0x12345678;
});
...
test('DataView：验证字节序一致性', () => {
  ...
  const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
  return view.getUint8(0) === 0 && view.getUint8(3) === 1;
});
...
test('DataView：负数表示', () => {
  ...
  buf.writeInt32BE(-1, 0);
  const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
  return view.getInt32(0, false) === -1;
});
...
test('DataView：最大值', () => {
  ...
  const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
  return view.getInt32(0, false) === 2147483647;
});
...
test('ArrayBuffer 偏移：验证隔离', () => {
  ...
  return buf1.readInt32BE(0) === 111 && buf2.readInt32BE(0) === 222;
});
...
test('byteOffset：验证正确位置', () => {
  ...
  return fullView.getInt32(4, false) === 123;
});
...
test('复杂嵌套：ArrayBuffer -> Uint8Array -> Buffer', () => {
  ...
  const buf = Buffer.from(u8.buffer, u8.byteOffset, u8.byteLength);
  buf.writeInt32BE(123, 0);
  return buf.readInt32BE(0) === 123 && u8[0] === 0 && u8[3] === 123;
});
...
test('长度边界：恰好 4 字节', () => {
  ...
  const buf = Buffer.from(ab);
  buf.writeInt32BE(123, 0);
  return buf.length === 4 && buf.readInt32BE(0) === 123;
});
...
test('长度边界：5 字节写在结尾', () => {
  ...
  return buf.readInt32BE(1) === 123;
});

// 与其他 write 方法的交互
test('方法混合：writeInt32BE 后 writeInt8', () => {
...
test('方法混合：writeInt8 后 writeInt32BE', () => {
  const buf = Buffer.allocUnsafe(5);
...
test('方法混合：writeInt32BE 覆盖 writeInt8', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeInt8(127, 0);
...
test('方法混合：writeInt16BE 后 writeInt32BE', () => {
...
test('fill 交互：写入后 fill', () => {
  ...
  buf.fill(0xFF, 0, 2);
  buf.fill(0xFF, 6, 8);
  return buf[0] === 0xFF && buf.readInt32BE(2) === 123 && buf[7] === 0xFF;
});
...
// Buffer.concat 交互
test('concat 交互：包含 writeInt32BE 的 Buffer', () => {
  const buf1 = Buffer.allocUnsafe(4);
  const buf2 = Buffer.allocUnsafe(4);
  buf1.writeInt32BE(111, 0);
  buf2.writeInt32BE(222, 0);
  const result = Buffer.concat([buf1, buf2]);
  return result.readInt32BE(0) === 111 && result.readInt32BE(4) === 222;
});
