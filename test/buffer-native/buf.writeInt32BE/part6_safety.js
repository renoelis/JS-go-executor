...
test('内存隔离：多个 Buffer 指向同一内存', () => {
  ...
  view1.writeInt32BE(0x11111111, 0);
...
test('并发安全：同一位置多次写入', () => {
  ...
  buf.writeInt32BE(222, 0);
  buf.writeInt32BE(333, 0);
...
test('并发安全：不同位置独立写入', () => {
  ...
  return buf.readInt32BE(0) === 111 && buf.readInt32BE(4) === 222 && buf.readInt32BE(8) === 333;
});
...
test('零拷贝：subarray 共享内存', () => {
  ...
  child.writeInt32BE(0x12345678, 0);
  return parent[0] === 0x12 && parent[3] === 0x78;
});
...
test('内存对齐：对齐地址写入', () => {
  ...
  buf.writeInt32BE(0x7A7A7A7A, 0);
  buf.writeInt32BE(0x7B7B7B7B, 4);
  buf.writeInt32BE(0x7C7C7C7C, 8);
  buf.writeInt32BE(0x7D7D7D7D, 12);
  return buf[0] === 0x7A && buf[4] === 0x7B && buf[8] === 0x7C && buf[12] === 0x7D;
});

test('内存对齐：非对齐地址写入', () => {
  const buf = Buffer.allocUnsafe(15);
  buf.writeInt32BE(0x7A7A7A7A, 1);
  buf.writeInt32BE(0x7B7B7B7B, 5);
  buf.writeInt32BE(0x7C7C7C7C, 9);
