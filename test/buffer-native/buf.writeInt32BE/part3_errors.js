...
test('this 不是 Buffer：普通对象（允许）', () => {
  ...
  return obj[0] === 0 && obj[1] === 0 && obj[2] === 0 && obj[3] === 123;
});

test('this 不是 Buffer：数组（允许）', () => {
  const arr = [0, 0, 0, 0];
  Buffer.prototype.writeInt32BE.call(arr, 123, 0);
  return arr[0] === 0 && arr[1] === 0 && arr[2] === 0 && arr[3] === 123;
});

test('this 不是 Buffer：null', () => {
  try {
    Buffer.prototype.writeInt32BE.call(null, 123, 0);
    return false;
  } catch (e) {
...
test('this 不是 Buffer：undefined', () => {
  try {
    Buffer.prototype.writeInt32BE.call(undefined, 123, 0);
    return false;
  } catch (e) {
    return e.message.includes('Buffer') || e.message.includes('Uint8Array') || e.message.includes('this') || e.message.includes('Cannot');
  }
