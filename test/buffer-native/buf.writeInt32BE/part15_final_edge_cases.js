...
test('writeInt32BE.length 应该是 1', () => {
  return Buffer.prototype.writeInt32BE.length === 1;
});

test('writeInt32BE.name 应该是 "writeInt32BE"', () => {
  return Buffer.prototype.writeInt32BE.name === 'writeInt32BE';
});

test('writeInt32BE 是一个函数', () => {
  return typeof Buffer.prototype.writeInt32BE === 'function';
});
...
test('value 超出范围应返回 RangeError', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.writeInt32BE(999999999999, 0);
    return false;
  } catch (e) {
    return e.name === 'RangeError' || e.message.includes('out of range');
  }
...
test('普通对象作为 this 可以写入', () => {
  ...
  Buffer.prototype.writeInt32BE.call(obj, 100, 0);
  return obj[0] === 0 && obj[1] === 0 && obj[2] === 0 && obj[3] === 100;
});

test('带额外属性的对象作为 this', () => {
  const obj = { 0: 0, 1: 0, 2: 0, 3: 0, length: 4, extra: 'data' };
  Buffer.prototype.writeInt32BE.call(obj, 200, 0);
  return obj[0] === 0 && obj[1] === 0 && obj[2] === 0 && obj[3] === 200 && obj.extra === 'data';
});
...
test('多次调用相同参数结果一致', () => {
  ...
  buf1.writeInt32BE(12345678, 0);
  buf2.writeInt32BE(12345678, 0);
  buf3.writeInt32BE(12345678, 0);
...
test('连续100次调用结果稳定', () => {
  const buf = Buffer.alloc(8);
  for (let i = 0; i < 100; i++) {
    buf.writeInt32BE(12345678, 0);
  }
  return buf.readInt32BE(0) === 12345678;
});
...
test('最大值写入最后位置', () => {
  const buf = Buffer.alloc(8);
  buf.writeInt32BE(2147483647, 4);
  return buf.readInt32BE(4) === 2147483647;
});

test('最小值写入最后位置', () => {
  const buf = Buffer.alloc(8);
  buf.writeInt32BE(-2147483648, 4);
  return buf.readInt32BE(4) === -2147483648;
});
...
test('修改原型方法不影响已有实例', () => {
  const buf = Buffer.alloc(8);
  const original = Buffer.prototype.writeInt32BE;
  
  buf.writeInt32BE(123, 0);
  const val1 = buf.readInt32BE(0);
  
  Buffer.prototype.writeInt32BE = function() {};
  Buffer.prototype.writeInt32BE = original;
  
  buf.writeInt32BE(456, 0);
  const val2 = buf.readInt32BE(0);
...
test('使用 bind 绑定 this', () => {
  const buf = Buffer.alloc(8);
  const boundWrite = buf.writeInt32BE.bind(buf);
  boundWrite(123, 0);
...
test('使用 call 指定 this', () => {
  const buf = Buffer.alloc(8);
  Buffer.prototype.writeInt32BE.call(buf, 456, 0);
  return buf.readInt32BE(0) === 456;
});

test('使用 apply 指定 this', () => {
  const buf = Buffer.alloc(8);
  Buffer.prototype.writeInt32BE.apply(buf, [789, 0]);
  return buf.readInt32BE(0) === 789;
});
...
test('从 slice 创建的 Buffer', () => {
  ...
  child.writeInt32BE(123, 0);
  return parent.readInt32BE(4) === 123;
});
...
test('交替正负数写入', () => {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    buf.writeInt32BE(value, i * 4);
  }
  return buf.readInt32BE(0) === 0 && buf.readInt32BE(4) === -1 && buf.readInt32BE(8) === 2;
});
