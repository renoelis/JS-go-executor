// buf.writeInt32BE() - 第2轮补充：对照官方文档补漏
...
// 官方文档：签名为 buf.writeInt32BE(value[, offset])
test('文档：默认 offset 为 0', () => {
  const buf = Buffer.allocUnsafe(4);
  const result = buf.writeInt32BE(123);
  return result === 4 && buf[3] === 123;
});
...
test('文档：负数使用补码表示', () => {
  ...
  return buf[3] === 0x9C && buf[2] === 0xFF && buf[1] === 0xFF && buf[0] === 0xFF;
});
...
test('文档：返回值可用于链式调用', () => {
  ...
  let off = 0;
  off = buf.writeInt32BE(111, off);
  off = buf.writeInt32BE(222, off);
  off = buf.writeInt32BE(333, off);
  return off === 12 && buf.readInt32BE(0) === 111 && buf.readInt32BE(4) === 222 && buf.readInt32BE(8) === 333;
});

// 官方文档：value 应为有效整数
test('文档：非整数转换为整数', () => {
  const buf = Buffer.allocUnsafe(4);
  buf.writeInt32BE(123.99, 0);
  const val = buf.readInt32BE(0);
  return val === 123;
});

// 官方文档：BE 表示高位字节在前
test('文档：Big-Endian 字节顺序', () => {
  const buf = Buffer.allocUnsafe(4);
...
test('文档：实例方法而非静态方法', () => {
  const buf = Buffer.allocUnsafe(4);
  return typeof buf.writeInt32BE === 'function' && typeof Buffer.writeInt32BE !== 'function';
});
