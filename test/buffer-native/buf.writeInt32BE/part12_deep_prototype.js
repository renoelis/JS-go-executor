// buf.writeInt32BE() - 深度补充：原型链与特殊对象
...
// 方法存在性检查
test('方法存在：writeInt32BE 在原型上', () => {
  return 'writeInt32BE' in Buffer.prototype;
});

test('方法存在：writeInt32BE 是函数', () => {
  return typeof Buffer.prototype.writeInt32BE === 'function';
});

test('方法存在：实例可访问', () => {
  const buf = Buffer.allocUnsafe(4);
  return typeof buf.writeInt32BE === 'function';
});

test('方法存在：不是静态方法', () => {
  return typeof Buffer.writeInt32BE === 'undefined';
});

// 方法属性
test('方法属性：length 属性', () => {
  return typeof Buffer.prototype.writeInt32BE.length === 'number';
});
...
// 继承与原型链
test('原型链：Buffer 实例包含 writeInt32BE 方法', () => {
  const buf = Buffer.allocUnsafe(4);
  return typeof buf.writeInt32BE === 'function' && buf instanceof Buffer;
});

test('原型链：方法不在实例自身', () => {
  const buf = Buffer.allocUnsafe(4);
  return !buf.hasOwnProperty('writeInt32BE');
});

test('原型链：方法可被覆盖（实例层面）', () => {
  const buf = Buffer.allocUnsafe(4);
  const original = buf.writeInt32BE;
  buf.writeInt32BE = null;
...
test('创建方式：Buffer.from(buffer)', () => {
  ...
  buf2.writeInt32BE(123, 0);
  return buf2.readInt32BE(0) === 123;
});
...
test('call：正常调用', () => {
  const buf = Buffer.allocUnsafe(4);
  Buffer.prototype.writeInt32BE.call(buf, 123, 0);
  return buf.readInt32BE(0) === 123;
});

test('apply：正常调用', () => {
  const buf = Buffer.allocUnsafe(4);
  Buffer.prototype.writeInt32BE.apply(buf, [123, 0]);
  return buf.readInt32BE(0) === 123;
});

test('bind：绑定后调用', () => {
  const buf = Buffer.allocUnsafe(4);
  const bound = Buffer.prototype.writeInt32BE.bind(buf);
  bound(123, 0);
  return buf.readInt32BE(0) === 123;
});
...
test('bind：部分参数绑定', () => {
  const buf = Buffer.allocUnsafe(4);
  const bound = Buffer.prototype.writeInt32BE.bind(buf, 123);
  bound(0);
  return buf.readInt32BE(0) === 123;
});
...
test('subarray：保持原型链', () => {
  ...
  return parent.readInt32BE(0) === 123;
});

test('subarray：方法仍可用', () => {
  const parent = Buffer.allocUnsafe(8);
  const child = parent.subarray(0, 4);
  return typeof child.writeInt32BE === 'function';
});
...
test('slice：保持原型链', () => {
  ...
  return typeof sliced.writeInt32BE === 'function';
});
