// buf.writeInt32BE() - 深度补充：错误消息与堆栈验证
...
// 堆栈包含函数名
test('堆栈：包含 writeInt32BE', () => {
  try {
    const buf = Buffer.allocUnsafe(4);
    buf.writeInt32BE(123, 5);
    return false;
  } catch (e) {
    return e.stack.includes('writeInt32BE');
  }
});
...
test('错误恢复：多次异常不影响正常操作', () => {
  ...
  try {
    buf.writeInt32BE(456, 10);
  } catch (e) {}
  buf.writeInt32BE(789, 0);
