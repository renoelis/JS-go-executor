// buf.readDoubleBE() - IEEE 754 标准测试
...
// IEEE 754 标准值
test('最小正规数', () => {
  const buf = Buffer.from([0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
  const result = buf.readDoubleBE(0);
  return result === 2.2250738585072014e-308;
});

test('最大次正规数', () => {
  const buf = Buffer.from([0x00, 0x0F, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]);
  const result = buf.readDoubleBE(0);
...
test('最小次正规数（MIN_VALUE）', () => {
  ...
  return buf.readDoubleBE(0) === 5e-324;
});

test('最大正数（MAX_VALUE）', () => {
  const buf = Buffer.from([0x7F, 0xEF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]);
...
test('符号位为 1（负数）', () => {
  const buf = Buffer.from([0xBF, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
  return buf.readDoubleBE(0) === -1.0;
});
...
test('指数全 1（无穷或 NaN）', () => {
  const buf1 = Buffer.from([0x7F, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
  const buf2 = Buffer.from([0x7F, 0xF8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
  return buf1.readDoubleBE(0) === Infinity && Number.isNaN(buf2.readDoubleBE(0));
});

// 尾数测试
test('尾数全 0', () => {
  const buf = Buffer.from([0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
  return buf.readDoubleBE(0) === 2.0;
});

test('尾数全 1', () => {
  const buf = Buffer.from([0x3F, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]);
  const result = buf.readDoubleBE(0);
...
test('2^10 = 1024', () => {
  const buf = Buffer.from([0x40, 0x90, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
  return buf.readDoubleBE(0) === 1024.0;
});

test('2^-1 = 0.5', () => {
  const buf = Buffer.from([0x3F, 0xE0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
  return buf.readDoubleBE(0) === 0.5;
});

test('2^-10 = 0.0009765625', () => {
  const buf = Buffer.from([0x3F, 0x50, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
  return buf.readDoubleBE(0) === 0.0009765625;
});
