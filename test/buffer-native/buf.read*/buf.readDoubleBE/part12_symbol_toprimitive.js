// buf.readDoubleBE() - Symbol.toPrimitive 和对象转换测试
...
test('offset 对象有 Symbol.toPrimitive 返回 0', () => {
  try {
    ...
    const result = buf.readDoubleBE(obj);
...
test('Symbol.toPrimitive 优先于 valueOf', () => {
  try {
    ...
    const result = buf.readDoubleBE(obj);
    return Math.abs(result - 200.5) < 0.01;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
});
...
test('offset 对象有 valueOf 返回 0', () => {
  try {
    ...
    const result = buf.readDoubleBE(obj);
    return Math.abs(result - 300.5) < 0.01;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
});
