...
test('MAX_SAFE_INTEGER', () => {
  const buf = Buffer.alloc(8);
  buf.writeDoubleBE(Number.MAX_SAFE_INTEGER, 0);
  return buf.readDoubleBE(0) === Number.MAX_SAFE_INTEGER;
});

test('MIN_SAFE_INTEGER', () => {
  const buf = Buffer.alloc(8);
  buf.writeDoubleBE(Number.MIN_SAFE_INTEGER, 0);
  return buf.readDoubleBE(0) === Number.MIN_SAFE_INTEGER;
});
...
test('Math.PI 精度', () => {
  ...
  return buf.readDoubleBE(0) === Math.PI;
});

test('Math.E 精度', () => {
  const buf = Buffer.alloc(8);
  buf.writeDoubleBE(Math.E, 0);
  return buf.readDoubleBE(0) === Math.E;
});

test('Math.SQRT2 精度', () => {
  const buf = Buffer.alloc(8);
  buf.writeDoubleBE(Math.SQRT2, 0);
  return buf.readDoubleBE(0) === Math.SQRT2;
});

test('Math.LN2 精度', () => {
  const buf = Buffer.alloc(8);
  buf.writeDoubleBE(Math.LN2, 0);
  return buf.readDoubleBE(0) === Math.LN2;
});
