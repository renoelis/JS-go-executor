...
test('undefined offset 使用默认值不抛错', () => {
  try {
    ...
    const result = buf.readDoubleBE(undefined);
...
test('超小 offset 抛出 RangeError', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readDoubleBE(Number.MIN_SAFE_INTEGER);
    return false;
  } catch (e) {
    return e.name === 'RangeError';
  }
