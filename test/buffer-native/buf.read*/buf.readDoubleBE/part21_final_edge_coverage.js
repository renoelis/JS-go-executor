...
test('offset = Promise.resolve(0)（应抛出 TypeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readDoubleBE(Promise.resolve(0));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = new ArrayBuffer(8)（应抛出 TypeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readDoubleBE(new ArrayBuffer(8));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = new TypeError()（应抛出 TypeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readDoubleBE(new TypeError());
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = Buffer.from([0])（应抛出 TypeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readDoubleBE(Buffer.from([0]));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = new Uint8Array([0])（应抛出 TypeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readDoubleBE(new Uint8Array([0]));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = new Int32Array([0])（应抛出 TypeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readDoubleBE(new Int32Array([0]));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = 0x8（十六进制 8）', () => {
  ...
  const result = buf.readDoubleBE(0x8);
  return Math.abs(result - 111.222) < 0.001;
});

test('offset = 0o10（八进制 8）', () => {
  const buf = Buffer.alloc(16);
  buf.writeDoubleBE(333.444, 8);
  const result = buf.readDoubleBE(0o10);
  return Math.abs(result - 333.444) < 0.001;
});

test('offset = 0b1000（二进制 8）', () => {
  const buf = Buffer.alloc(16);
  buf.writeDoubleBE(555.666, 8);
  const result = buf.readDoubleBE(0b1000);
...
test('读取 1MB Buffer 的最后 8 字节', () => {
  ...
  buf.writeDoubleBE(999.999, size - 8);
  const result = buf.readDoubleBE(size - 8);
...
test('重复读取 1000 次同一位置', () => {
  const buf = Buffer.alloc(8);
  buf.writeDoubleBE(123.456789, 0);
  const expected = buf.readDoubleBE(0);
...
test('并发读取 100 个不同位置', () => {
  ...
  for (let i = 0; i < 100; i++) {
    const result = buf.readDoubleBE(i * 8);
