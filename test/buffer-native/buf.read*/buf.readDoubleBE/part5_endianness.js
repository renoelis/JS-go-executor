// buf.readDoubleBE() - 字节序（Endianness）专项测试
...
test('BE vs LE - 不同值', () => {
  ...
  return be !== le && be === 8.20788039913184e-304;
});

test('BE vs LE - 对称值', () => {
  const buf = Buffer.from([0x40, 0x09, 0x21, 0xFB, 0x54, 0x44, 0x2D, 0x18]);
  const be = buf.readDoubleBE(0);
  const reversedBuf = Buffer.from([0x18, 0x2D, 0x44, 0x54, 0xFB, 0x21, 0x09, 0x40]);
  const le = reversedBuf.readDoubleLE(0);
  return Math.abs(be - le) < 1e-15;
});

// IEEE 754 格式验证（Big-Endian）
...
test('Math.E 的 IEEE 754 表示', () => {
  const buf = Buffer.from([0x40, 0x05, 0xBF, 0x0A, 0x8B, 0x14, 0x57, 0x6A]);
  return Math.abs(buf.readDoubleBE(0) - Math.E) < 1e-15;
});

// Big-Endian 特性：高位字节在前
test('第一个字节影响符号位和指数', () => {
  const buf1 = Buffer.from([0x3F, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
  const buf2 = Buffer.from([0xBF, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]);
  return buf1.readDoubleBE(0) === 1.0 && buf2.readDoubleBE(0) === -1.0;
});

// 写入 BE 读取 BE
test('writeDoubleBE + readDoubleBE 一致性', () => {
...
// 写入 LE 读取 BE（应该不同）
test('writeDoubleLE + readDoubleBE 不同', () => {
  const buf = Buffer.alloc(8);
  const value = 123456789.987654321;
  buf.writeDoubleLE(value, 0);
  const beValue = buf.readDoubleBE(0);
...
test('递增序列 0x01-0x08', () => {
  const buf = Buffer.from([0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]);
  return buf.readDoubleBE(0) === 8.20788039913184e-304;
});
