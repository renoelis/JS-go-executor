// buf.readDoubleBE() - Offset 参数完整验证
...
test('offset = buf.length - 8（最大有效值）', () => {
  ...
  return Math.abs(buf.readDoubleBE(8) - 999.999) < 0.001;
});
...
test('offset = undefined（应使用默认值 0）', () => {
  ...
  return Math.abs(buf.readDoubleBE(undefined) - 777.777) < 0.001;
});

test('offset = null（应抛出 TypeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readDoubleBE(null);
    return false;
  } catch (e) {
...
test('offset = Number.MAX_SAFE_INTEGER（应抛出 RangeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readDoubleBE(Number.MAX_SAFE_INTEGER);
    return false;
  } catch (e) {
    return e.name === 'RangeError';
  }
