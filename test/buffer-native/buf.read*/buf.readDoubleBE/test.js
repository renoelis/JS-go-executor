// buf.readDoubleBE() - Complete Tests
...
test('读取正浮点数', () => {
  ...
  buf.writeDoubleBE(3.14159265359, 0);
  return Math.abs(buf.readDoubleBE(0) - 3.14159265359) < 0.000000001;
});

test('读取负浮点数', () => {
  const buf = Buffer.alloc(8);
  buf.writeDoubleBE(-2.718281828, 0);
  return Math.abs(buf.readDoubleBE(0) - (-2.718281828)) < 0.000000001;
});
...
test('高精度往返', () => {
  ...
  return Math.abs(buf.readDoubleBE(0) - value) < 1e-15;
});
