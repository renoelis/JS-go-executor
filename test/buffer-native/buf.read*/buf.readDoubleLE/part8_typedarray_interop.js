// buf.readDoubleLE() - TypedArray 互操作测试
...
test('从 ArrayBuffer 创建并读取', () => {
  ...
  view.set([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF0, 0x3F]);
  const buf = Buffer.from(ab);
  return buf.readDoubleLE(0) === 1.0;
});

// 与 DataView 对比
test('Buffer.readDoubleLE vs DataView.getFloat64 LE', () => {
  const arr = new Uint8Array([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40]);
  const buf = Buffer.from(arr);
  const dv = new DataView(arr.buffer);
  return buf.readDoubleLE(0) === dv.getFloat64(0, true);
});

test('多个位置对比', () => {
  const arr = new Uint8Array(16);
  arr.set([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF0, 0x3F], 0);
  arr.set([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40], 8);
  const buf = Buffer.from(arr);
  const dv = new DataView(arr.buffer);
  return buf.readDoubleLE(0) === dv.getFloat64(0, true) &&
         buf.readDoubleLE(8) === dv.getFloat64(8, true);
});
...
test('与 Float64Array 互操作（需注意字节序）', () => {
  ...
  // 注意：Float64Array 使用系统字节序，通常是 LE
...
test('通过 buffer 属性访问 ArrayBuffer', () => {
  ...
  return Math.abs(dv.getFloat64(0, true) - 2.718281828) < 1e-9;
});
...
test('Buffer subarray', () => {
  ...
  buf.writeDoubleLE(123.456, 8);
  const sub = buf.subarray(8, 16);
  return Math.abs(sub.readDoubleLE(0) - 123.456) < 0.001;
});

test('Buffer slice', () => {
  const buf = Buffer.alloc(16);
  buf.writeDoubleLE(789.012, 8);
  const slice = buf.slice(8, 16);
  return Math.abs(slice.readDoubleLE(0) - 789.012) < 0.001;
});

// 跨 TypedArray 类型
test('Int8Array 转 Buffer 读取', () => {
  const i8arr = new Int8Array([0x18, 0x2D, 0x44, 0x54, -5, 0x21, 0x09, 0x40]);
...
test('Buffer.from(buffer)', () => {
  ...
  buf1.writeDoubleLE(999.999, 0);
  const buf2 = Buffer.from(buf1);
  return Math.abs(buf2.readDoubleLE(0) - 999.999) < 0.001;
});

test('Buffer.from(arrayBuffer, offset, length)', () => {
  const ab = new ArrayBuffer(16);
  const view = new Uint8Array(ab);
  view.set([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x40], 4);
