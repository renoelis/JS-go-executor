// buf.readDoubleLE() - Offset 参数完整验证
...
test('offset = buf.length - 8（最大有效值）', () => {
  ...
  return Math.abs(buf.readDoubleLE(8) - 999.999) < 0.001;
});
...
test('offset = undefined（应使用默认值 0）', () => {
  ...
  return Math.abs(buf.readDoubleLE(undefined) - 777.777) < 0.001;
});
