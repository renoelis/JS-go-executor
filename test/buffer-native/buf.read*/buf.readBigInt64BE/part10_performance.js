...
test('连续读取 100 个值', () => {
  ...
  for (let i = 0; i < 100; i++) {
    if (buf.readBigInt64BE(i * 8) !== BigInt(i)) {
      return false;
    }
  }
  return true;
...
test('大 Buffer 随机位置读取', () => {
  ...
  for (let i = 0; i < positions.length; i++) {
    buf.writeBigInt64BE(values[i], positions[i]);
  }
  
  for (let i = 0; i < positions.length; i++) {
    if (buf.readBigInt64BE(positions[i]) !== values[i]) {
      return false;
    }
  }
  return true;
...
test('多位置同时读取', () => {
  ...
  const results = [];
  for (let i = 0; i < 8; i++) {
    results.push(buf.readBigInt64BE(i * 8));
  }
...
test('边界附近密集读取', () => {
  ...
  // 在 buffer 末尾附近写入多个值
  buf.writeBigInt64BE(111n, 84);
  buf.writeBigInt64BE(222n, 92);
  
  return buf.readBigInt64BE(84) === 111n && 
         buf.readBigInt64BE(92) === 222n;
});
...
test('稀疏位置读取', () => {
  ...
  for (let i = 0; i < positions.length; i++) {
    buf.writeBigInt64BE(BigInt(positions[i]), positions[i]);
  }
  
  for (let i = 0; i < positions.length; i++) {
    if (buf.readBigInt64BE(positions[i]) !== BigInt(positions[i])) {
      return false;
    }
  }
  return true;
...
test('顺序和逆序读取一致性', () => {
  ...
  // 顺序读取
  for (let i = 0; i < 10; i++) {
    if (buf.readBigInt64BE(i * 8) !== values[i]) {
      return false;
    }
  }
