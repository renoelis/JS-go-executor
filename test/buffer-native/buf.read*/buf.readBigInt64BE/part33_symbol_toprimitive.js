// buf.readBigInt64BE() - Symbol.toPrimitive 和特殊对象测试
...
test('offset 为 Date 对象（valueOf 返回时间戳）', () => {
  try {
    ...
    const result = buf.readBigInt64BE(date);
    return result === 0n;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('offset 为 Function', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigInt64BE(function() { return 0; });
    return false;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('offset 为 WeakMap 对象', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigInt64BE(new WeakMap());
    return false;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('offset 为 WeakSet 对象', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigInt64BE(new WeakSet());
    return false;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('offset 为 Promise 对象', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigInt64BE(Promise.resolve(0));
    return false;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('offset 为 Error 对象', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigInt64BE(new Error('test'));
    return false;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
