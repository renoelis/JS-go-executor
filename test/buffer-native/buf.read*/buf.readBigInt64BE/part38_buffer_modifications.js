// buf.readBigInt64BE() - Buffer 修改和并发测试
...
test('读取后修改 Buffer 不影响已读取的值', () => {
  ...
  const value1 = buf.readBigInt64BE(0);
  buf.writeBigInt64BE(200n, 0);
  const value2 = buf.readBigInt64BE(0);
...
test('读取时其他位置的值不影响读取结果', () => {
  ...
  const value1 = buf.readBigInt64BE(0);
  const value2 = buf.readBigInt64BE(8);
  return value1 === 100n && value2 === 200n;
});
...
test('连续读取相同位置返回相同值', () => {
  ...
  const v2 = buf.readBigInt64BE(0);
  const v3 = buf.readBigInt64BE(0);
...
test('fill(0x80) 后读取', () => {
  const buf = Buffer.alloc(8);
  buf.fill(0x80);
  return buf.readBigInt64BE(0) === -9187201950435737472n;
});
...
test('从另一个 Buffer copy 后读取', () => {
  ...
  buf1.writeBigInt64BE(99999n, 0);
  buf1.copy(buf2, 0, 0, 8);
  return buf2.readBigInt64BE(0) === 99999n;
});
...
test('部分 copy 后读取', () => {
  ...
  const expected = 0x0102030400000000n;
  return buf2.readBigInt64BE(0) === expected;
});
...
test('原 Buffer 修改影响 slice', () => {
  ...
  const slice = buf.slice(0, 8);
  buf.writeBigInt64BE(300n, 0);
  return slice.readBigInt64BE(0) === 300n;
});
...
test('原 Buffer 修改影响 subarray', () => {
  ...
  return sub.readBigInt64BE(0) === 500n;
});
