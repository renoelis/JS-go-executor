// buf.readBigInt64BE() - this 上下文绑定测试
...
test('使用 call 调用', () => {
  ...
  const result = buf.readBigInt64BE.call(buf, 0);
  return result === 200n;
});
...
test('使用 apply 调用', () => {
  ...
  const result = buf.readBigInt64BE.apply(buf, [0]);
  return result === 300n;
});
...
test('bind 到不同的 Buffer', () => {
  ...
  const readFromBuf2 = buf1.readBigInt64BE.bind(buf2);
...
test('箭头函数中调用', () => {
  ...
  const arrow = () => buf.readBigInt64BE(0);
  return arrow() === 500n;
});
...
test('解构赋值后调用需要绑定 this', () => {
  ...
  const result = readBigInt64BE.call(buf, 0);
  return result === 700n;
});
...
test('多次 bind 调用', () => {
  ...
  const bound1 = buf.readBigInt64BE.bind(buf);
