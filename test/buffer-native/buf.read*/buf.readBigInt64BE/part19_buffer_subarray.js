// buf.readBigInt64BE() - Buffer subarray 和 slice 测试
...
test('从 subarray 读取', () => {
  ...
  buf.writeBigInt64BE(100n, 8);
  const sub = buf.subarray(8, 16);
  return sub.readBigInt64BE(0) === 100n;
});
...
test('subarray 的 subarray', () => {
  ...
  buf.writeBigInt64BE(500n, 8);
  const sub1 = buf.subarray(0, 16);
  const sub2 = sub1.subarray(8, 16);
  return sub2.readBigInt64BE(0) === 500n;
});
...
test('subarray 恰好 8 字节', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64BE(600n, 4);
  const sub = buf.subarray(4, 12);
  return sub.readBigInt64BE(0) === 600n;
});

// 从大 Buffer 的中间 subarray
test('从大 Buffer 的中间 subarray', () => {
  const buf = Buffer.alloc(1024);
  buf.writeBigInt64BE(700n, 512);
  const sub = buf.subarray(512, 520);
  return sub.readBigInt64BE(0) === 700n;
});
...
test('负索引 subarray', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64BE(800n, 8);
  const sub = buf.subarray(-8);
  return sub.readBigInt64BE(0) === 800n;
});

// 负索引范围 subarray
test('负索引范围 subarray', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64BE(900n, 4);
  const sub = buf.subarray(-12, -4);
  return sub.readBigInt64BE(0) === 900n;
});

// Buffer.from 创建的 subarray
test('Buffer.from 创建的 subarray', () => {
  const buf = Buffer.from([0, 0, 0, 0, 0, 0, 0, 100, 0, 0, 0, 0, 0, 0, 0, 200]);
  const sub = buf.subarray(0, 8);
  return sub.readBigInt64BE(0) === 100n;
});
...
test('多层嵌套 subarray', () => {
  ...
  buf.writeBigInt64BE(1000n, 16);
  const sub1 = buf.subarray(8, 32);
  const sub2 = sub1.subarray(8, 16);
  return sub2.readBigInt64BE(0) === 1000n;
});
...
test('subarray 后原 Buffer 修改', () => {
  ...
  return sub.readBigInt64BE(0) === 1100n;
});
