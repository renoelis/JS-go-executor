// buf.readBigInt64BE() - 冻结和密封的 Buffer 测试
...
test('从冻结的数组创建 Buffer', () => {
  const arr = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00];
  Object.freeze(arr);
...
test('不可扩展的 Buffer 读取极值', () => {
  const buf = Buffer.alloc(8);
  buf.writeBigInt64BE(-9223372036854775808n, 0);
  Object.preventExtensions(buf);
...
test('preventExtensions 的 slice 读取', () => {
  ...
  buf.writeBigInt64BE(888n, 4);
  const sliced = buf.slice(4, 12);
  Object.preventExtensions(sliced);
  return sliced.readBigInt64BE(0) === 888n;
});
...
test('preventExtensions 的 subarray 读取', () => {
  ...
  return sub.readBigInt64BE(0) === 999n;
});
