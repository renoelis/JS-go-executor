// buf.readBigInt64BE() - 不同 Buffer 类型测试
...
test('从 ArrayBuffer 创建的 Buffer', () => {
  ...
  view.setBigInt64(0, 12345n, false); // false = big-endian
...
test('slice 后的 Buffer', () => {
  ...
  buf.writeBigInt64BE(777n, 4);
  const sliced = buf.slice(4, 12);
  return sliced.readBigInt64BE(0) === 777n;
});
...
test('subarray 后的 Buffer', () => {
  ...
  return sub.readBigInt64BE(0) === 666n;
});

// 从 Buffer.concat 创建
test('从 Buffer.concat 创建', () => {
  const buf1 = Buffer.from([0x00, 0x00, 0x00, 0x00]);
  const buf2 = Buffer.from([0x00, 0x00, 0x00, 0x0A]);
...
test('16 字节 Buffer，从 offset=4 读取', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64BE(555n, 4);
  return buf.readBigInt64BE(4) === 555n;
});
