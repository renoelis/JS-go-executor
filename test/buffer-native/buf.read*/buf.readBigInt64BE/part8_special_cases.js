...
test('传入多余参数（应忽略）', () => {
  ...
  return buf.readBigInt64BE(0, 'extra', 123, {}) === 100n;
});
...
test('offset = Symbol（应抛出错误）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigInt64BE(Symbol('test'));
    return false;
  } catch (e) {
...
test('offset = BigInt（应抛出错误或转换）', () => {
  try {
    ...
    const result = buf.readBigInt64BE(0n);
    return result === 100n;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('offset = Number.MIN_VALUE（应抛出 RangeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigInt64BE(Number.MIN_VALUE);
    return false;
  } catch (e) {
    return e.name === 'RangeError';
  }
...
test('offset = Number.MAX_VALUE（应抛出 RangeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigInt64BE(Number.MAX_VALUE);
    return false;
  } catch (e) {
    return e.name === 'RangeError';
  }
...
test('offset = Number.EPSILON（应抛出 RangeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigInt64BE(Number.EPSILON);
    return false;
  } catch (e) {
    return e.name === 'RangeError';
  }
