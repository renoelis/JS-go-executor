...
test('严格模式下箭头函数', () => {
  ...
  const read = (b, o) => b.readBigInt64BE(o);
...
test('严格模式下 call', () => {
  ...
  return buf.readBigInt64BE.call(buf, 0) === 500n;
});
...
test('严格模式下 apply', () => {
  ...
  return buf.readBigInt64BE.apply(buf, [0]) === 600n;
});
...
test('严格模式下对象方法', () => {
  const obj = {
    ...
    read: function() {
      'use strict';
      this.buffer.writeBigInt64BE(700n, 0);
