// buf.readBigInt64BE() - SharedArrayBuffer 和特殊 ArrayBuffer 测试
...
test('从 ArrayBuffer 创建并读取', () => {
  ...
  view.setBigInt64(0, 12345n, false); // Big-Endian
...
test('从 ArrayBuffer.slice 创建并读取', () => {
  ...
  const view = new DataView(ab);
  view.setBigInt64(8, 99999n, false);
...
test('从 Uint8Array.buffer 创建并读取', () => {
  ...
  view.setBigInt64(4, 77777n, false);
  const buf = Buffer.from(arr.buffer, 4, 8);
  return buf.readBigInt64BE(0) === 77777n;
});
...
test('多个视图共享 ArrayBuffer', () => {
  ...
  view1.setBigInt64(0, 11111n, false);
  view1.setBigInt64(8, 22222n, false);
  
  const buf1 = Buffer.from(ab, 0, 8);
  const buf2 = Buffer.from(ab, 8, 8);
  
  return buf1.readBigInt64BE(0) === 11111n && buf2.readBigInt64BE(0) === 22222n;
});
...
test('从 Uint16Array 创建并读取', () => {
  ...
  const view = new DataView(arr.buffer);
  view.setBigInt64(0, 55555n, false);
  const buf = Buffer.from(arr.buffer);
  return buf.readBigInt64BE(0) === 55555n;
});
...
test('从 Float64Array.buffer 创建并读取', () => {
  ...
  const view = new DataView(arr.buffer);
  view.setBigInt64(0, 88888n, false);
  const buf = Buffer.from(arr.buffer, 0, 8);
  return buf.readBigInt64BE(0) === 88888n;
});
...
test('从 DataView.buffer 创建并读取', () => {
  ...
  dv.setBigInt64(0, 33333n, false);
  const buf = Buffer.from(dv.buffer);
  return buf.readBigInt64BE(0) === 33333n;
});
...
test('修改 ArrayBuffer 后 Buffer 同步更新', () => {
  ...
  const buf = Buffer.from(ab);
  const view = new DataView(ab);
  view.setBigInt64(0, 44444n, false);
  return buf.readBigInt64BE(0) === 44444n;
});
