// buf.readBigInt64BE() - 极端 Buffer 大小和场景测试
...
test('大 Buffer（1KB）在末尾位置读取', () => {
  const buf = Buffer.alloc(1024);
  buf.writeBigInt64BE(789n, 1016);
  return buf.readBigInt64BE(1016) === 789n;
});
...
test('大 Buffer（10MB）读取', () => {
  try {
    ...
    buf.writeBigInt64BE(1234n, 0);
    return buf.readBigInt64BE(0) === 1234n;
  } catch (e) {
    // 某些环境可能内存不足
...
test('大 Buffer 中多个位置读取', () => {
  ...
  buf.writeBigInt64BE(200n, 512);
  buf.writeBigInt64BE(300n, 1016);
  return buf.readBigInt64BE(0) === 100n &&
         buf.readBigInt64BE(512) === 200n &&
         buf.readBigInt64BE(1016) === 300n;
});
...
test('Buffer.allocUnsafe 可以读取', () => {
  ...
  buf.writeBigInt64BE(555n, 0);
  return buf.readBigInt64BE(0) === 555n;
});

test('Buffer.allocUnsafe 大 Buffer', () => {
  const buf = Buffer.allocUnsafe(1024);
  buf.writeBigInt64BE(666n, 100);
  return buf.readBigInt64BE(100) === 666n;
});
...
test('连续分配多个 Buffer 并读取', () => {
  ...
  return buffers.every((buf, i) => buf.readBigInt64BE(0) === BigInt(i));
});
...
test('Buffer.concat 后读取', () => {
  ...
  return combined.readBigInt64BE(0) === 111n && combined.readBigInt64BE(8) === 222n;
});
