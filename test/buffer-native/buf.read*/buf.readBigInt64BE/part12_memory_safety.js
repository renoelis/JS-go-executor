// buf.readBigInt64BE() - 内存安全和数据完整性测试
...
test('部分修改 Buffer 后读取', () => {
  ...
  // 0xFF02030405060708 是负数，因为最高位是 1
  return result === -0x0fdfcfbfaf9f8f8n;
});
...
test('修改中间字节后读取', () => {
  ...
  buf[4] = 0xAA;
  const result = buf.readBigInt64BE(0);
  return result === 0x01020304AA060708n;
});
...
test('fill 部分区域后读取', () => {
  ...
  buf.fill(0xFF, 0, 4);
  const result = buf.readBigInt64BE(0);
  // 0xFFFFFFFF00000000 是负数
  return result === -0x100000000n;
});

// copy 操作
test('copy 后读取', () => {
  const buf1 = Buffer.alloc(8);
  buf1.writeBigInt64BE(12345n, 0);
  const buf2 = Buffer.alloc(8);
  buf1.copy(buf2);
  return buf2.readBigInt64BE(0) === 12345n;
});
...
test('部分 copy 后读取', () => {
  ...
  const result = buf2.readBigInt64BE(0);
  return result === 0x0102030400000000n;
});

// slice 共享内存
test('slice 共享内存 - 修改原 Buffer', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64BE(100n, 4);
  const sliced = buf.slice(4, 12);
  buf.writeBigInt64BE(200n, 4);
  return sliced.readBigInt64BE(0) === 200n;
});
...
test('slice 共享内存 - 修改 slice', () => {
  ...
  buf.writeBigInt64BE(100n, 4);
  const sliced = buf.slice(4, 12);
  sliced.writeBigInt64BE(300n, 0);
  return buf.readBigInt64BE(4) === 300n;
});

// subarray 共享内存
test('subarray 共享内存 - 修改原 Buffer', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64BE(111n, 8);
  const sub = buf.subarray(8, 16);
  buf.writeBigInt64BE(222n, 8);
  return sub.readBigInt64BE(0) === 222n;
});
...
test('subarray 共享内存 - 修改 subarray', () => {
  ...
  buf.writeBigInt64BE(111n, 8);
  const sub = buf.subarray(8, 16);
  sub.writeBigInt64BE(333n, 0);
  return buf.readBigInt64BE(8) === 333n;
});
...
test('Buffer.from(buffer) 创建新副本', () => {
  ...
  return buf2.readBigInt64BE(0) === 100n;
});
...
test('交叉覆盖写入', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64BE(0x0102030405060708n, 0);
  buf.writeBigInt64BE(0x1112131415161718n, 4);
  // offset=4 的写入会覆盖 offset=0 的后 4 字节
  const r1 = buf.readBigInt64BE(0);
  const r2 = buf.readBigInt64BE(4);
