// buf.readBigUInt64BE() - Offset 参数完整验证
...
test('offset 默认值为 0', () => {
  ...
  return buf.readBigUInt64BE() === 12345n;
});
...
test('offset = buf.length - 8（最大有效值）', () => {
  ...
  return buf.readBigUInt64BE(8) === 99999n;
});
...
test('offset = undefined（应使用默认值 0）', () => {
  ...
  return buf.readBigUInt64BE(undefined) === 777n;
});

test('offset = null（应抛出 TypeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64BE(null);
    return false;
  } catch (e) {
...
test('offset = Number.MAX_SAFE_INTEGER（应抛出 RangeError）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64BE(Number.MAX_SAFE_INTEGER);
    return false;
  } catch (e) {
    return e.name === 'RangeError';
  }
