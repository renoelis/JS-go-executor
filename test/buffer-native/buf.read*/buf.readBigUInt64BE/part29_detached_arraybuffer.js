// buf.readBigUInt64BE() - Detached ArrayBuffer 测试
...
test('Buffer 复制后原 ArrayBuffer 可以读取', () => {
  ...
  return buf2.readBigUInt64BE(0) === 789n;
});
...
test('Buffer.slice 后读取', () => {
  ...
  return slice.readBigUInt64BE(0) === 333n;
});
...
test('修改原 Buffer 影响 subarray', () => {
  ...
  return sub.readBigUInt64BE(0) === 888n;
});
...
test('修改 subarray 影响原 Buffer', () => {
  ...
  const sub = buf.subarray(0, 8);
  sub.writeBigUInt64BE(1111n, 0);
  return buf.readBigUInt64BE(0) === 1111n;
});

test('slice 和 subarray 在 Node.js 中都返回共享内存视图', () => {
  const buf = Buffer.alloc(8);
  buf.writeBigUInt64BE(2222n, 0);
  const slice = buf.slice(0, 8);
  slice.writeBigUInt64BE(3333n, 0);
  // 在 Node.js 中，slice 现在也返回共享内存的视图，与 subarray 行为一致
  return buf.readBigUInt64BE(0) === 3333n && slice.readBigUInt64BE(0) === 3333n;
});

test('从 TypedArray 的 buffer 属性创建 Buffer', () => {
  const ta = new Uint8Array(8);
  const buf = Buffer.from(ta.buffer);
  buf.writeBigUInt64BE(4444n, 0);
  return buf.readBigUInt64BE(0) === 4444n;
});

test('Buffer.concat 后读取', () => {
  const buf1 = Buffer.alloc(8);
  const buf2 = Buffer.alloc(8);
  buf1.writeBigUInt64BE(5555n, 0);
  buf2.writeBigUInt64BE(6666n, 0);
  const concat = Buffer.concat([buf1, buf2]);
  return concat.readBigUInt64BE(0) === 5555n && concat.readBigUInt64BE(8) === 6666n;
});
...
test('ArrayBuffer 边界对齐测试', () => {
  ...
  buf.writeBigUInt64BE(7777n, 0);
  return buf.readBigUInt64BE(0) === 7777n;
});

test('多层 subarray 测试', () => {
  const buf = Buffer.alloc(32);
  buf.writeBigUInt64BE(8888n, 8);
  const sub1 = buf.subarray(4, 20);
  const sub2 = sub1.subarray(4, 12);
  return sub2.readBigUInt64BE(0) === 8888n;
});

test('Buffer.from 复制 subarray', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigUInt64BE(9999n, 0);
  const sub = buf.subarray(0, 8);
  const copy = Buffer.from(sub);
  buf.writeBigUInt64BE(1010n, 0);
  return copy.readBigUInt64BE(0) === 9999n && buf.readBigUInt64BE(0) === 1010n;
});
