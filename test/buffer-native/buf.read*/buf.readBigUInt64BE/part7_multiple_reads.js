...
test('同一位置多次读取一致性', () => {
  ...
  const r1 = buf.readBigUInt64BE(0);
  const r2 = buf.readBigUInt64BE(0);
  const r3 = buf.readBigUInt64BE(0);
...
test('逆序读取', () => {
  ...
  buf.writeBigUInt64BE(222n, 8);
  buf.writeBigUInt64BE(333n, 16);
  return buf.readBigUInt64BE(16) === 333n &&
         buf.readBigUInt64BE(8) === 222n &&
         buf.readBigUInt64BE(0) === 111n;
});
...
test('重复读取 100 次', () => {
  const buf = Buffer.alloc(8);
  buf.writeBigUInt64BE(99999n, 0);
  for (let i = 0; i < 100; i++) {
    if (buf.readBigUInt64BE(0) !== 99999n) {
      return false;
    }
  }
  return true;
...
test('循环读取多个值', () => {
  ...
  for (let i = 0; i < values.length; i++) {
    buf.writeBigUInt64BE(values[i], i * 8);
  }
  for (let i = 0; i < values.length; i++) {
    if (buf.readBigUInt64BE(i * 8) !== values[i]) {
      return false;
    }
  }
  return true;
...
test('读取 1000 个值', () => {
  ...
  for (let i = 0; i < count; i++) {
    if (buf.readBigUInt64BE(i * 8) !== BigInt(i)) {
      return false;
    }
  }
  return true;
