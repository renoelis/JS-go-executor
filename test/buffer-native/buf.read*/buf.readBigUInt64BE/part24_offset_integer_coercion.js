// buf.readBigUInt64BE() - offset 整数强制转换测试
...
test('offset 为小数 0.0（应视为 0）', () => {
  ...
  try {
    const result = buf.readBigUInt64BE(0.0);
    return result === 123n;
  } catch (e) {
    // 如果抛出错误也是合理的
...
test('offset 为 1.0（整数形式的浮点数，应抛出错误）', () => {
  ...
  try {
    const result = buf.readBigUInt64BE(1.0);
    // Node.js 接受整数形式的浮点数
    return result === 789n;
  } catch (e) {
    // 如果抛出错误也是合理的
...
test('offset 为 2.0（整数形式的浮点数，应抛出错误）', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigUInt64BE(999n, 2);
  try {
    const result = buf.readBigUInt64BE(2.0);
    // Node.js 接受整数形式的浮点数
    return result === 999n;
  } catch (e) {
    // 如果抛出错误也是合理的
...
test('offset 为 -Number.EPSILON（应视为 0 或抛出错误）', () => {
  ...
  try {
    const result = buf.readBigUInt64BE(-Number.EPSILON);
    // 如果成功，应该读取到值
    return result === 111n;
  } catch (e) {
    // 抛出错误也是合理的
