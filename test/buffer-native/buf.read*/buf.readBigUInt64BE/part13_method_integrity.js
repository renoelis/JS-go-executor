// buf.readBigUInt64BE() - 方法完整性和属性测试
...
test('readBigUInt64BE 属性存在于原型上', () => {
  return 'readBigUInt64BE' in Buffer.prototype;
});

test('readBigUInt64BE 可以被枚举（通过 for...in）', () => {
  const props = [];
  for (const key in Buffer.prototype) {
    props.push(key);
  }
  return props.includes('readBigUInt64BE');
});

test('readBigUInt64BE 可以被删除和重新赋值', () => {
  const original = Buffer.prototype.readBigUInt64BE;
  try {
    // 尝试删除
    delete Buffer.prototype.readBigUInt64BE;
    const deleted = !Buffer.prototype.readBigUInt64BE;
    // 恢复
    Buffer.prototype.readBigUInt64BE = original;
    return deleted && Buffer.prototype.readBigUInt64BE === original;
  } catch (e) {
    // 如果不可配置，会抛出错误
    Buffer.prototype.readBigUInt64BE = original;
    return false;
...
// 返回值类型
test('readBigUInt64BE 返回 BigInt', () => {
  const buf = Buffer.alloc(8);
  const result = buf.readBigUInt64BE(0);
  return typeof result === 'bigint';
});

test('readBigUInt64BE 返回值是 BigInt 实例', () => {
  const buf = Buffer.alloc(8);
  const result = buf.readBigUInt64BE(0);
...
// 方法不修改 this
test('readBigUInt64BE 不修改原 Buffer', () => {
