...
test('在非 Buffer 对象上调用（应抛出错误）', () => {
  try {
    ...
    Buffer.prototype.readBigUInt64BE.call(notBuffer, 0);
    return false;
  } catch (e) {
...
test('在 null 上调用（应抛出 TypeError）', () => {
  try {
    Buffer.prototype.readBigUInt64BE.call(null, 0);
    return false;
  } catch (e) {
...
test('在 undefined 上调用（应抛出 TypeError）', () => {
  try {
    Buffer.prototype.readBigUInt64BE.call(undefined, 0);
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('在普通对象上调用（应抛出错误）', () => {
  try {
    Buffer.prototype.readBigUInt64BE.call({}, 0);
    return false;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('在数组上调用（应返回 0n）', () => {
  const result = Buffer.prototype.readBigUInt64BE.call([0, 0, 0, 0, 0, 0, 0, 0], 0);
  return result === 0n;
});
...
test('无参数调用（使用默认 offset = 0）', () => {
  ...
  return buf.readBigUInt64BE() === 123n;
});

test('多余参数被忽略', () => {
  const buf = Buffer.alloc(8);
  buf.writeBigUInt64BE(456n, 0);
  return buf.readBigUInt64BE(0, 'extra', 'params') === 456n;
});
