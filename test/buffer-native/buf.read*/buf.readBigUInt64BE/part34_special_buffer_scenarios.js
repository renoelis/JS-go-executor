// buf.readBigUInt64BE() - 特殊 Buffer 场景测试
...
test('Buffer.subarray 创建的视图可以读取', () => {
  ...
  return sub.readBigUInt64BE(0) === 666n;
});
...
test('Buffer.subarray 创建的视图与原 Buffer 共享内存', () => {
  ...
  return sub.readBigUInt64BE(0) === 777n;
});
...
test('Buffer.slice 创建的副本可以读取', () => {
  ...
  return slice.readBigUInt64BE(0) === 888n;
});
...
test('修改原 Buffer 不影响 slice（在 Node.js v4+ slice 也是视图）', () => {
  ...
  const slice = buf.slice(8);
  const before = slice.readBigUInt64BE(0);
  buf.writeBigUInt64BE(1111n, 8);
  const after = slice.readBigUInt64BE(0);
...
test('部分 fill 后的 Buffer 可以正确读取', () => {
  ...
  return buf.readBigUInt64BE(0) === 0n &&
         buf.readBigUInt64BE(8) === 0xFFFFFFFFFFFFFFFFn;
});
...
test('Buffer.compare 后仍可以正确读取', () => {
  const buf1 = Buffer.alloc(8);
  const buf2 = Buffer.alloc(8);
  buf1.writeBigUInt64BE(123n, 0);
  buf2.writeBigUInt64BE(456n, 0);
  Buffer.compare(buf1, buf2);
  return buf1.readBigUInt64BE(0) === 123n &&
         buf2.readBigUInt64BE(0) === 456n;
});
...
test('Buffer.equals 后仍可以正确读取', () => {
  const buf1 = Buffer.alloc(8);
  const buf2 = Buffer.alloc(8);
  buf1.writeBigUInt64BE(789n, 0);
  buf2.writeBigUInt64BE(789n, 0);
  buf1.equals(buf2);
  return buf1.readBigUInt64BE(0) === 789n &&
         buf2.readBigUInt64BE(0) === 789n;
});
...
test('Buffer.swap64 后可以读取交换后的值', () => {
  ...
  buf.swap64();
  return buf.readBigUInt64BE(0) === 0x0807060504030201n;
});
...
test('Buffer.copy 后目标 Buffer 可以读取', () => {
  ...
  src.writeBigUInt64BE(456n, 0);
  src.copy(dst, 0, 0, 8);
  return dst.readBigUInt64BE(0) === 456n;
});

test('Buffer.copy 部分复制后可以读取', () => {
  const src = Buffer.alloc(16);
  const dst = Buffer.alloc(8);
  src.writeBigUInt64BE(789n, 8);
  src.copy(dst, 0, 8, 16);
  return dst.readBigUInt64BE(0) === 789n;
});
