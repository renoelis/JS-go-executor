...
test('offset = new WeakMap() 应该抛出错误', () => {
  try {
    ...
    buf.readBigUInt64BE(new WeakMap());
    return false;
  } catch (e) {
...
test('offset = new WeakSet() 应该抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64BE(new WeakSet());
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = Promise.resolve(0) 应该抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64BE(Promise.resolve(0));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = new Error() 应该抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64BE(new Error('test'));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = new ArrayBuffer(8) 应该抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64BE(new ArrayBuffer(8));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = new DataView(new ArrayBuffer(8)) 应该抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64BE(new DataView(new ArrayBuffer(8)));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = new Uint8Array([0]) 应该抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64BE(new Uint8Array([0]));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('非严格模式下读取正常工作', () => {
  ...
  return buf.readBigUInt64BE(0) === 888n;
});
...
test('offset = 类数组对象应该抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64BE({ 0: 0, length: 1 });
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = Buffer.from([0]) 应该抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64BE(Buffer.from([0]));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset = new Number(-0) 等同于 0', () => {
  try {
    ...
    const result = buf.readBigUInt64BE(new Number(-0));
    return result === 777n;
  } catch (e) {
    // 某些实现可能抛出 TypeError
    return e.name === 'TypeError';
