...
# buf.readInt16BE / readInt16LE 完整测试运行脚本
# 用于验证所有测试文件在 Go + goja 服务中的表现

set -e

BASE_DIR="/Users/Code/Go-product/Flow-codeblock_goja"
TEST_DIR="$BASE_DIR/test/buffer-native/buf.read*/buf.readInt16BE、buf.readInt16LE"
...
echo "=========================================="
echo "buf.readInt16BE / readInt16LE API 完整测试"
echo "=========================================="
echo ""
...
if [ "$total_tests" -gt 0 ]; then
  ...
  if [ "$total_failed" -eq 0 ]; then
    echo ""
    echo "🎉 所有测试通过！buf.readInt16BE/LE API 与 Node.js v25.0.0 完全兼容！"
    exit 0
  else
    echo ""
