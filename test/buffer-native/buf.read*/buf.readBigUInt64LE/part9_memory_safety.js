...
test('未对齐的 offset（offset = 1）', () => {
  ...
  buf.writeBigUInt64LE(77777n, 1);
  return buf.readBigUInt64LE(1) === 77777n;
});

test('未对齐的 offset（offset = 3）', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigUInt64LE(88888n, 3);
  return buf.readBigUInt64LE(3) === 88888n;
});

test('未对齐的 offset（offset = 7）', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigUInt64LE(99999n, 7);
  return buf.readBigUInt64LE(7) === 99999n;
});
...
test('Buffer.slice 独立性', () => {
  ...
  sliced.writeBigUInt64LE(222n, 0);
...
test('大 Buffer（1MB）', () => {
  ...
  buf.writeBigUInt64LE(123456789n, offset);
  return buf.readBigUInt64LE(offset) === 123456789n;
});
