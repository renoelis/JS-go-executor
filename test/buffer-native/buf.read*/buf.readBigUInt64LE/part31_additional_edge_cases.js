...
test('Buffer.from(array) 可以读取', () => {
  const buf = Buffer.from([10, 0, 0, 0, 0, 0, 0, 0]);
...
test('Buffer.from(buffer) 可以读取', () => {
  ...
  return buf2.readBigUInt64LE(0) === 111n;
});
...
test('Buffer 长度 16 字节时 offset=0 和 offset=8 可以读取', () => {
  ...
  return buf.readBigUInt64LE(0) === 333n && buf.readBigUInt64LE(8) === 444n;
});

test('Buffer 长度 15 字节时最大 offset 为 7', () => {
  const buf = Buffer.alloc(15);
  buf.writeBigUInt64LE(555n, 7);
  return buf.readBigUInt64LE(7) === 555n;
});
...
test('Buffer.write 后读取', () => {
  const buf = Buffer.alloc(8);
  buf.write('\x00\x01\x00\x00\x00\x00\x00\x00', 0, 'binary');
  return buf.readBigUInt64LE(0) === 256n;
});

test('Buffer.copy 后读取', () => {
  const buf1 = Buffer.alloc(8);
  buf1.writeBigUInt64LE(666n, 0);
  const buf2 = Buffer.alloc(8);
  buf1.copy(buf2);
  return buf2.readBigUInt64LE(0) === 666n;
});

test('Buffer.copy 部分数据后读取', () => {
  const buf1 = Buffer.alloc(16);
  buf1.writeBigUInt64LE(777n, 0);
  buf1.writeBigUInt64LE(888n, 8);
  const buf2 = Buffer.alloc(8);
  buf1.copy(buf2, 0, 8, 16);
  return buf2.readBigUInt64LE(0) === 888n;
});
...
test('offset = 0.0 等同于 offset = 0', () => {
  ...
  return buf.readBigUInt64LE(0.0) === 1010n;
});

test('offset = Number("0") 可以工作', () => {
  const buf = Buffer.alloc(8);
  buf.writeBigUInt64LE(1111n, 0);
  return buf.readBigUInt64LE(Number("0")) === 1111n;
});

// 测试与其他 read 方法的交互
test('writeBigUInt64LE + readBigUInt64LE 往返', () => {
  const buf = Buffer.alloc(8);
...
test('writeBigInt64BE + readBigUInt64LE 读取相同字节', () => {
  const buf = Buffer.alloc(8);
  buf.writeBigInt64BE(-1n, 0);
  return buf.readBigUInt64LE(0) === 18446744073709551615n;
});

test('writeUInt32LE 两次 + readBigUInt64LE', () => {
  const buf = Buffer.alloc(8);
  buf.writeUInt32LE(0x12345678, 0);
  buf.writeUInt32LE(0x9ABCDEF0, 4);
  return buf.readBigUInt64LE(0) === 0x9ABCDEF012345678n;
});
...
test('Buffer.alloc 指定 fill 值为数组', () => {
  ...
  return buf.readBigUInt64LE(0) === 0x0101010101010101n;
});
...
test('连续读取不同位置', () => {
  ...
  buf.writeBigUInt64LE(200n, 8);
  buf.writeBigUInt64LE(300n, 16);
  return buf.readBigUInt64LE(0) === 100n &&
         buf.readBigUInt64LE(8) === 200n &&
         buf.readBigUInt64LE(16) === 300n;
});

test('循环读取', () => {
  const buf = Buffer.alloc(32);
  for (let i = 0; i < 4; i++) {
    buf.writeBigUInt64LE(BigInt(i * 100), i * 8);
  }
  let allPass = true;
  for (let i = 0; i < 4; i++) {
    if (buf.readBigUInt64LE(i * 8) !== BigInt(i * 100)) {
      allPass = false;
    }
  }
  return allPass;
