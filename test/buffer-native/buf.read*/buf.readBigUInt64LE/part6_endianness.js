// buf.readBigUInt64LE() - 字节序（Endianness）专项测试
...
test('LE vs BE - 256n', () => {
  ...
  return le === 281474976710656n && be === 256n && le !== be;
});
...
test('递增序列 0x01-0x08', () => {
  const buf = Buffer.from([0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]);
  return buf.readBigUInt64LE(0) === 578437695752307201n;
});

// 递减序列
test('递减序列 0x08-0x01', () => {
  const buf = Buffer.from([0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01]);
  return buf.readBigUInt64LE(0) === 72623859790382856n;
});

// 写入 BE 读取 BE
test('writeBigUInt64LE + readBigUInt64LE 一致性', () => {
...
// 写入 BE 读取 LE（应该不同）
test('writeBigUInt64BE + readBigUInt64LE 不同', () => {
  const buf = Buffer.alloc(8);
  const value = 1234567890123456789n;
  buf.writeBigUInt64BE(value, 0);
  const leValue = buf.readBigUInt64LE(0);
