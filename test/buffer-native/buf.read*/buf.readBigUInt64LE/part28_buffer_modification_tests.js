// buf.readBigUInt64LE() - Buffer 修改和状态测试
...
test('读取后修改 Buffer 再读取', () => {
  ...
  const first = buf.readBigUInt64LE(0);
  buf.writeBigUInt64LE(222n, 0);
  const second = buf.readBigUInt64LE(0);
...
test('部分修改 Buffer 后读取', () => {
  ...
  return result === 0xFFFFFFFFFFFFFF00n;
});
...
test('修改单个字节影响读取结果', () => {
  ...
  buf[7] = 0x01; // 设置最高字节
  return buf.readBigUInt64LE(0) === 72057594037927936n; // 0x01 << 56
});
...
test('交替读写多次', () => {
  const buf = Buffer.alloc(8);
  for (let i = 0; i < 5; i++) {
    buf.writeBigUInt64LE(BigInt(i * 100), 0);
    if (buf.readBigUInt64LE(0) !== BigInt(i * 100)) {
      return false;
    }
  }
  return true;
...
test('fill 部分区域后读取', () => {
  const buf = Buffer.alloc(8);
  buf.fill(0x00);
  buf.fill(0xFF, 4, 8); // 填充高位字节 (4-7)
  return buf.readBigUInt64LE(0) === 0xFFFFFFFF00000000n;
});
...
test('copy 到另一个 Buffer 后读取', () => {
  ...
  src.writeBigUInt64LE(12345n, 0);
  src.copy(dst);
  return dst.readBigUInt64LE(0) === 12345n;
});

test('copy 部分数据后读取', () => {
  const src = Buffer.alloc(16);
  const dst = Buffer.alloc(8);
  src.writeBigUInt64LE(11111n, 0);
  src.writeBigUInt64LE(22222n, 8);
  src.copy(dst, 0, 8, 16);
  return dst.readBigUInt64LE(0) === 22222n;
});
...
test('使用 set 部分修改后读取', () => {
  ...
  buf.set(arr, 0); // 设置最低两个字节
  return buf.readBigUInt64LE(0) === 0x000000000000FFFFn;
});

// Buffer 反转测试
test('reverse 后读取值改变', () => {
  const buf = Buffer.from([0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]);
  const before = buf.readBigUInt64LE(0); // LE: 0x0807060504030201
  buf.reverse(); // 变成 [0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01]
  const after = buf.readBigUInt64LE(0); // LE: 0x0102030405060708
  return before !== after && after === 0x0102030405060708n;
});
...
// 写入后立即读取
test('writeBigUInt64LE 后立即 readBigUInt64LE', () => {
  const buf = Buffer.alloc(8);
