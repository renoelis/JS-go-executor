...
test('offset = NaN（应抛出错误）', () => { try { Buffer.alloc(8).readBigUInt64LE(NaN); return false; } catch (e) { return e.name === 'RangeError' || e.name === 'TypeError'; } });
test('offset = 字符串（应抛出错误）', () => { try { Buffer.alloc(8).readBigUInt64LE('0'); return false; } catch (e) { return e.name === 'TypeError'; } });
test('offset = 浮点数（应抛出错误）', () => { try { Buffer.alloc(8).readBigUInt64LE(0.5); return false; } catch (e) { return e.name === 'RangeError'; } });
