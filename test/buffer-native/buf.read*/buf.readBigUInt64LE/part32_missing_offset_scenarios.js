// buf.readBigUInt64LE() - 遗漏的 offset 场景测试
...
test('offset 为 Number.NaN 应抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64LE(Number.NaN);
    return false;
  } catch (e) {
...
test('offset 为 parseFloat("abc") (NaN) 应抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64LE(parseFloat("abc"));
    return false;
  } catch (e) {
    return e.name === 'RangeError' || e.name === 'TypeError';
  }
...
test('offset 为 Number.POSITIVE_INFINITY 应抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64LE(Number.POSITIVE_INFINITY);
    return false;
  } catch (e) {
...
test('offset 为 Number.NEGATIVE_INFINITY 应抛出错误', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64LE(Number.NEGATIVE_INFINITY);
    return false;
  } catch (e) {
    return e.name === 'RangeError';
  }
...
test('offset 为正则表达式 /8/ 应通过 valueOf 转换', () => {
  try {
    const buf = Buffer.alloc(16);
    buf.writeBigUInt64LE(1234n, 8);
    // 正则表达式的 valueOf 返回自身，toString 返回字符串
    const regex = /8/;
    buf.readBigUInt64LE(regex);
...
test('offset 为 Boolean(true) 应抛出 TypeError', () => {
  try {
    ...
    buf.readBigUInt64LE(Boolean(true));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
...
test('offset 为 Boolean(false) 应抛出 TypeError', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigUInt64LE(Boolean(false));
    return false;
  } catch (e) {
    return e.name === 'TypeError';
  }
