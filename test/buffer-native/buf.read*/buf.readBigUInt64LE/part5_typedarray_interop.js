// buf.readBigUInt64LE() - TypedArray 互操作性测试
...
test('从 ArrayBuffer 创建的 Buffer', () => {
  ...
  view.setBigUint64(0, 12345n, true); // Little-Endian
...
test('与 DataView.getBigUint64 一致（Little-Endian）', () => {
  const buf = Buffer.alloc(8);
  buf.writeBigUInt64LE(9876543210n, 0);
...
test('从 subarray 读取', () => {
  ...
  buf.writeBigUInt64LE(55555n, 8);
  const sub = buf.subarray(8);
  return sub.readBigUInt64LE(0) === 55555n;
});

test('subarray 的 offset', () => {
  const buf = Buffer.alloc(24);
  buf.writeBigUInt64LE(77777n, 16);
  const sub = buf.subarray(8);
  return sub.readBigUInt64LE(8) === 77777n;
});

// Buffer.slice 测试
test('从 slice 读取', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigUInt64LE(66666n, 8);
  const sliced = buf.slice(8);
  return sliced.readBigUInt64LE(0) === 66666n;
});
