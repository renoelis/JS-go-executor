...
test('offset = undefined（应使用默认值 0）', () => {
  ...
  return buf.readInt8(undefined) === 127;
});
...
test('offset = true（应抛出错误或转为 1）', () => {
  try {
    ...
    const result = buf.readInt8(true);
    // 某些实现可能将 true 转为 1
    return result === 50;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
...
test('offset = false（应抛出错误或转为 0）', () => {
  try {
    ...
    const result = buf.readInt8(false);
    // 某些实现可能将 false 转为 0
    return result === 100;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
...
test('offset = 1.0（整数浮点数，应被接受）', () => {
  const buf = Buffer.from([100, 50]);
  return buf.readInt8(1.0) === 50;
});

test('offset = 0.0（应被接受）', () => {
  const buf = Buffer.from([100]);
  return buf.readInt8(0.0) === 100;
});
...
test('offset = Number.MAX_SAFE_INTEGER（应抛出错误）', () => {
  try {
    const buf = Buffer.from([100]);
    buf.readInt8(Number.MAX_SAFE_INTEGER);
    return false;
  } catch (e) {
    return e.name === 'RangeError';
  }
...
test('offset = Number.MIN_SAFE_INTEGER（应抛出错误）', () => {
  try {
    const buf = Buffer.from([100]);
    buf.readInt8(Number.MIN_SAFE_INTEGER);
    return false;
  } catch (e) {
    return e.name === 'RangeError';
  }
