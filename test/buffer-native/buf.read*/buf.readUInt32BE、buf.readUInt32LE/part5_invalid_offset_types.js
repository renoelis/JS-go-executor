...
test('BE: offset = undefined (应使用默认值 0)', () => {
  ...
  return buf.readUInt32BE(undefined) === 0x12345678;
});

test('LE: offset = undefined (应使用默认值 0)', () => {
  const buf = Buffer.from([0x12, 0x34, 0x56, 0x78]);
  return buf.readUInt32LE(undefined) === 0x78563412;
});
