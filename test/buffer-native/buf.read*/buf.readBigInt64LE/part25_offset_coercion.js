// buf.readBigInt64LE() - offset 参数类型强制转换测试
...
test('offset 为 Number 包装对象', () => {
  try {
    ...
    const result = buf.readBigInt64LE(new Number(0));
    return result === 300n;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('offset 为 String 包装对象', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigInt64LE(new String('0'));
    return false;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('offset 为 Boolean 包装对象 true', () => {
  try {
    ...
    buf.readBigInt64LE(new Boolean(true));
    return false;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('offset 为 Boolean 包装对象 false', () => {
  try {
    ...
    const result = buf.readBigInt64LE(new Boolean(false));
    return result === 400n;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('offset 为空数组', () => {
  try {
    ...
    const result = buf.readBigInt64LE([]);
    return result === 500n;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
