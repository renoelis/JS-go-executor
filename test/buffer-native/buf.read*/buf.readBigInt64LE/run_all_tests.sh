...
# buf.readBigInt64LE() 完整测试运行脚本
# 用于验证 readBigInt64LE 方法在 Go + goja 服务中的表现

set -e

BASE_DIR="/Users/Code/Go-product/Flow-codeblock_goja"
TEST_DIR="$BASE_DIR/test/buffer-native/buf.read*/buf.readBigInt64LE"
...
tests=(
  ...
  "part39_error_codes.js"
  "part40_buffer_sources.js"
  "part41_bigint_offset.js"
  "part42_getter_setter.js"
  "part43_zero_length_buffer.js"
)
...
if [ "$total_tests" -gt 0 ]; then
  ...
  if [ "$total_failed" -eq 0 ]; then
    echo ""
    echo "🎉 所有测试通过！buf.readBigInt64LE() 与 Node.js v25.0.0 完全兼容！"
    exit 0
  else
    echo ""
