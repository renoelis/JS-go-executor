...
// 方法存在性
test('readBigInt64LE 方法存在于 Buffer.prototype', () => {
  return typeof Buffer.prototype.readBigInt64LE === 'function';
});

test('readBigInt64LE 方法存在于 Buffer 实例', () => {
  const buf = Buffer.alloc(8);
  return typeof buf.readBigInt64LE === 'function';
});

// 方法名称（goja 中可能返回 Go 函数签名）
test('readBigInt64LE 方法名称包含标识', () => {
  const name = Buffer.prototype.readBigInt64LE.name;
  // Node.js 返回 'readBigInt64LE'，goja 返回完整的 Go 函数签名
  return typeof name === 'string' && name.length > 0;
});

// 方法长度（参数个数）
test('readBigInt64LE 方法参数个数', () => {
  // Node.js 中 readBigInt64LE 接受 offset 参数
...
test('方法可以被赋值给变量', () => {
  ...
  const readMethod = buf.readBigInt64LE;
...
test('通过 Buffer.prototype 调用', () => {
  ...
  const result = Buffer.prototype.readBigInt64LE.call(buf, 0);
  return result === 300n;
});
...
test('方法在不同实例间共享', () => {
  ...
  return buf1.readBigInt64LE === buf2.readBigInt64LE;
});

// toString 调用
test('方法 toString 返回函数定义', () => {
  const str = Buffer.prototype.readBigInt64LE.toString();
...
test('多次调用同一方法引用', () => {
  ...
  const read = buf.readBigInt64LE.bind(buf);
  const r1 = read(0);
  const r2 = read(0);
...
test('方法可以作为回调传递', () => {
  ...
  const result = callMethod(Buffer.prototype.readBigInt64LE, buf, 0);
  return result === 600n;
});
...
test('方法引用可存储在数组中', () => {
  ...
  const methods = [buf.readBigInt64LE];
...
test('方法引用可存储在对象中', () => {
  ...
  const obj = { read: buf.readBigInt64LE };
