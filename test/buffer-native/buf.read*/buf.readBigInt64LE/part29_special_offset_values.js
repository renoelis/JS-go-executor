// buf.readBigInt64LE() - 特殊 offset 值测试
...
test('offset = 0.0（应等同于 0）', () => {
  ...
  return buf.readBigInt64LE(0.0) === 789n;
});
...
test('offset = 0e0（应等同于 0）', () => {
  ...
  return buf.readBigInt64LE(0e0) === 111n;
});

test('offset = 8e0（应等同于 8）', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64LE(222n, 8);
  return buf.readBigInt64LE(8e0) === 222n;
});
...
test('offset = 1.6e1（等于 16，有效）', () => {
  ...
  return buf.readBigInt64LE(1.6e1) === 333n;
});
...
test('offset = 0o10（八进制 8）', () => {
  ...
  return buf.readBigInt64LE(0o10) === 333n;
});

test('offset = 0x8（十六进制 8）', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64LE(444n, 8);
  return buf.readBigInt64LE(0x8) === 444n;
});

test('offset = 0b1000（二进制 8）', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64LE(555n, 8);
  return buf.readBigInt64LE(0b1000) === 555n;
});
...
test('offset = 0.0000000001（应抛出错误）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigInt64LE(0.0000000001);
    return false;
  } catch (e) {
    return e.name === 'RangeError';
  }
...
test('offset = 0.9999999999（应抛出错误）', () => {
  try {
    const buf = Buffer.alloc(8);
    buf.readBigInt64LE(0.9999999999);
    return false;
  } catch (e) {
    return e.name === 'RangeError';
  }
...
test('offset = 4 + 4（应等同于 8）', () => {
  ...
  return buf.readBigInt64LE(4 + 4) === 666n;
});

test('offset = 16 / 2（应等同于 8）', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64LE(777n, 8);
  return buf.readBigInt64LE(16 / 2) === 777n;
});

test('offset = 2 * 4（应等同于 8）', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64LE(888n, 8);
  return buf.readBigInt64LE(2 * 4) === 888n;
});

test('offset = 10 - 2（应等同于 8）', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64LE(999n, 8);
  return buf.readBigInt64LE(10 - 2) === 999n;
});

// 位运算结果
test('offset = 16 >> 1（应等同于 8）', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64LE(1111n, 8);
  return buf.readBigInt64LE(16 >> 1) === 1111n;
});

test('offset = 4 << 1（应等同于 8）', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64LE(2222n, 8);
  return buf.readBigInt64LE(4 << 1) === 2222n;
});
