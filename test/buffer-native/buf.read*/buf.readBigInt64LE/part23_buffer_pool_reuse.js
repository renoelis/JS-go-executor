// buf.readBigInt64LE() - Buffer 池化和重用测试
...
test('多次 allocUnsafe 独立性', () => {
  ...
  return buf1.readBigInt64LE(0) === 111n && buf2.readBigInt64LE(0) === 222n;
});

// Buffer.concat 后读取
test('Buffer.concat 后读取', () => {
  const buf1 = Buffer.from([0x64, 0x00, 0x00, 0x00]);
  const buf2 = Buffer.from([0x00, 0x00, 0x00, 0x00]);
  const combined = Buffer.concat([buf1, buf2]);
  return combined.readBigInt64LE(0) === 100n;
});
...
test('Buffer.concat 多个 Buffer 后读取', () => {
  ...
  return combined.readBigInt64LE(0) === 200n;
});
...
test('Buffer.concat 单个 Buffer', () => {
  ...
  return combined.readBigInt64LE(0) === 300n;
});
...
test('Buffer.copy 后读取', () => {
  ...
  src.writeBigInt64LE(12345n, 0);
  
  const dest = Buffer.alloc(8);
  src.copy(dest);
  
  return dest.readBigInt64LE(0) === 12345n;
});

// Buffer copy 部分
test('Buffer.copy 部分后读取', () => {
  const src = Buffer.alloc(16);
  src.writeBigInt64LE(11111n, 0);
  src.writeBigInt64LE(22222n, 8);
  
  const dest = Buffer.alloc(8);
  src.copy(dest, 0, 8, 16);
  
  return dest.readBigInt64LE(0) === 22222n;
});
...
test('Buffer.swap64 后读取', () => {
  ...
  buf.swap64();
  return buf.readBigInt64LE(0) === 0x0807060504030201n;
});
