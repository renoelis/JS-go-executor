// buf.readBigInt64LE() - 方法链式调用和组合测试
...
test('读取多个值并求和', () => {
  ...
  const sum = buf.readBigInt64LE(0) + buf.readBigInt64LE(8) + buf.readBigInt64LE(16);
  return sum === 60n;
});
...
test('读取多个值并比较', () => {
  ...
  return buf.readBigInt64LE(0) < buf.readBigInt64LE(8);
});
...
test('读取值并用于数组索引', () => {
  ...
  const arr = ['a', 'b', 'c', 'd'];
  const index = Number(buf.readBigInt64LE(0));
  return arr[index] === 'c';
});
...
test('读取值用于 for 循环次数', () => {
  ...
  const count = Number(buf.readBigInt64LE(0));
  let sum = 0;
...
test('读取值用于 while 循环条件', () => {
  ...
  let value = buf.readBigInt64LE(0);
  let count = 0;
  while (value > 0n) {
    count++;
    value--;
...
test('读取值用于数组解构', () => {
  const buf = Buffer.alloc(16);
  buf.writeBigInt64LE(11n, 0);
  buf.writeBigInt64LE(22n, 8);
  
  const [a, b] = [buf.readBigInt64LE(0), buf.readBigInt64LE(8)];
  return a === 11n && b === 22n;
});
