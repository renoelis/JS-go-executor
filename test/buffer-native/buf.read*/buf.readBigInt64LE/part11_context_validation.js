...
test('字符串调用（应抛出错误）', () => {
  try {
    ...
    Buffer.prototype.readBigInt64LE.call(str, 0);
    return false;
  } catch (e) {
...
test('数字调用（应抛出错误）', () => {
  try {
    Buffer.prototype.readBigInt64LE.call(12345678, 0);
    return false;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('使用 bind 绑定非 Buffer（应抛出错误）', () => {
  try {
    const notBuffer = {};
    const boundRead = Buffer.prototype.readBigInt64LE.bind(notBuffer);
    boundRead(0);
    return false;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
...
test('使用 apply 调用', () => {
  ...
  return Buffer.prototype.readBigInt64LE.apply(buf, [0]) === 456n;
});

test('使用 apply 调用非 Buffer（应抛出错误）', () => {
  try {
    Buffer.prototype.readBigInt64LE.apply({}, [0]);
    return false;
  } catch (e) {
    return e.name === 'TypeError' || e.name === 'RangeError';
  }
...
test('使用 call 调用', () => {
  ...
  return Buffer.prototype.readBigInt64LE.call(buf, 0) === 789n;
});
