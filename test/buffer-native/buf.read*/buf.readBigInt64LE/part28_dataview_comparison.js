// buf.readBigInt64LE() - 与 DataView 对比测试
...
test('Buffer 与 DataView 读取正数一致', () => {
  ...
  const bufValue = buf.readBigInt64LE(0);
  const dvValue = dv.getBigInt64(0, true); // false = little-endian
...
test('Buffer 与 DataView 读取负数一致', () => {
  const buf = Buffer.alloc(8);
  buf.writeBigInt64LE(-9999n, 0);
  
  const dv = new DataView(buf.buffer, buf.byteOffset, buf.length);
  const bufValue = buf.readBigInt64LE(0);
  const dvValue = dv.getBigInt64(0, true);
...
test('Buffer 与 DataView 读取不同 offset 一致', () => {
  ...
  const buf1 = buf.readBigInt64LE(0);
  const dv1 = dv.getBigInt64(0, true);
  const buf2 = buf.readBigInt64LE(8);
  const dv2 = dv.getBigInt64(8, true);
...
test('Buffer 与 DataView 读取特殊字节序一致', () => {
  ...
  return bufValue === dvValue && bufValue === 578437695752307201n;
});
...
test('Buffer 与 DataView 读取随机值一致', () => {
  ...
  return bufValue === dvValue && bufValue === -1090226688147180526n;
});
