...
test('组合参数 - 大 BUFFER + 小 fill + utf8 encoding', () => {
  const buf = Buffer.allocUnsafeSlow(100, 'X', 'utf8');
  return buf.length === 100 && buf.every(b => b === 88);
});

test('组合参数 - 小 BUFFER + 长 fill + ascii encoding', () => {
  const buf = Buffer.allocUnsafeSlow(3, 'HELLO', 'ascii');
  return buf.toString() === 'HEL';
});

test('组合参数 - fill 为 Uint8Array + 大 BUFFER', () => {
  const u8 = new Uint8Array([72, 69, 76, 76, 79]); // HELLO
  const buf = Buffer.allocUnsafeSlow(15, u8);
  return buf.toString() === 'HELLOHELLOHELLO';
});

test('组合参数 - fill 为 Buffer 实例 + 十六进制 encoding', () => {
  const fillBuf = Buffer.from('Test');
  const buf = Buffer.allocUnsafeSlow(12, fillBuf);
  return buf.toString() === 'TestTestTest';
});

test('组合参数 - size 为字符串数字形式 + fill 为 number + encoding 为 latin1', () => {
  const buf = Buffer.allocUnsafeSlow('16', 65, 'latin1');
  return buf.length === 16 && buf.every(b => b === 65);
});

test('组合参数 - size 为带前导零的数值 + fill 为空字符', () => {
  const buf = Buffer.allocUnsafeSlow(5, '');
  return buf.length === 5;
});

test('组合参数 - size 为 Number.toString() 解析的浮点 + fill 为 object', () => {
  const buf = Buffer.allocUnsafeSlow(Number('256.9'), { toString: () => 'Z' });
  return buf.length === 256 && buf[0] === 90;
});

test('组合参数 - size 为十六进制字符串 + fill 为无效字符串 + 编码为 utf16le', () => {
  const buf = Buffer.allocUnsafeSlow('0x20', '�', 'utf16le');
  return buf.length === 32;
});

test('组合参数 - size 转换为整数边界 + fill 为 view', () => {
...
test('组合参数 - size 为浮点字符串 + fill 为 null + encoding 省略', () => {
  const buf = Buffer.allocUnsafeSlow('10.0', null);
  return buf.length === 10;
});

test('组合参数 - 多类型混合（fill 顺序 vs overflow）', () => {
  const fill = Buffer.from('ABCDE');
  const buf = Buffer.allocUnsafeSlow(13, fill);
  return buf.length === 13 && buf.toString() === 'ABCDEABCDEABC';
});

test('组合参数 - size 为对象转换链 + 多个 encoding 校准', () => {
  const tests = [
    { size: { valueOf: () => 8 }, fill: 'A', encoding: 'binary' },
    { size: { toString: () => '8' }, fill: Buffer.from('B'), encoding: undefined },
...
test('组合参数 - size 类型混合 + Buffer 池化策略对比双用', () => {
  ...
  baseSizes.forEach(size => {
    const buf1 = Buffer.allocUnsafeSlow(size, 'A');                                      // 慢策略
    const buf2 = Buffer.allocUnsafeSlow(size, Buffer.from('B'));                        // 慢策略
    const buf3 = Buffer.allocUnsafeSlow(size, new Uint8Array([67]));                   // 慢策略
...
test('组合参数 - 全类型对齐：fill + encoding + size 混合', () => {
  ...
  matrix.forEach(cfg => {
    const buf = Buffer.allocUnsafeSlow(cfg.size, cfg.fill, cfg.encoding);
