// buf.writeInt16LE() - 第9轮补充：高级边界场景和特殊行为
...
test('offset 为对象（即使有 valueOf）抛出错误', () => {
  try {
    ...
    buf.writeInt16LE(100, offsetObj);
    return false;
  } catch (e) {
...
test('value 为 getter 属性对象', () => {
  ...
  buf.writeInt16LE(obj, 0);
  return buf.readInt16LE(0) === 150 && callCount > 0;
});
...
test('value 对象的 valueOf 是 getter', () => {
  ...
  buf.writeInt16LE(obj, 0);
  return buf.readInt16LE(0) === 250 && called;
});
...
test('冻结的数组作为 this 抛出错误', () => {
  try {
    ...
    Buffer.prototype.writeInt16LE.call(arr, 100, 0);
    return false;
  } catch (e) {
...
test('返回值是数字类型', () => {
  const buf = Buffer.alloc(10);
  const result = buf.writeInt16LE(100, 0);
...
test('返回值不能链式调用 Buffer 方法', () => {
  const buf = Buffer.alloc(10);
  const result = buf.writeInt16LE(100, 0);
  return typeof result.writeInt16LE === 'undefined';
});
...
test('多次写入同一值保持幂等性', () => {
  const buf = Buffer.alloc(10);
  for (let i = 0; i < 100; i++) {
    buf.writeInt16LE(999, 2);
  }
  return buf.readInt16LE(2) === 999;
});

// 重叠写入测试
test('writeInt16LE 后 writeInt8 部分覆盖', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(0x1234, 0); // LE: [0x34, 0x12]
  buf.writeInt8(0x56, 0); // 覆盖第一个字节
  return buf[0] === 0x56 && buf[1] === 0x12;
});

test('writeInt8 后 writeInt16LE 完全覆盖', () => {
  const buf = Buffer.alloc(10);
...
test('相邻写入不互相影响', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(0x1234, 0); // LE: [0x34, 0x12]
  buf.writeInt16LE(0x5678, 2); // LE: [0x78, 0x56]
  return buf[0] === 0x34 && buf[1] === 0x12 &&
...
test('字符串值带前后空格', () => {
  ...
  return buf.readInt16LE(0) === 123;
});

test('字符串值带制表符', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE('\t456\t', 0);
  return buf.readInt16LE(0) === 456;
});

test('字符串值带换行符', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE('\n789\n', 0);
  return buf.readInt16LE(0) === 789;
});

test('字符串 "0x10" 解析为 0', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE('0x10', 0);
...
test('接近 0.5 的小数', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(0.49999, 0);
  return buf.readInt16LE(0) === 0;
});

test('接近 1 的小数', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(0.99999, 0);
  return buf.readInt16LE(0) === 0;
});

test('1.00001 截断为 1', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(1.00001, 0);
...
test('-0.5 截断为 0', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(-0.5, 0);
  return buf.readInt16LE(0) === 0;
});

test('-0.9 截断为 0', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(-0.9, 0);
  return buf.readInt16LE(0) === 0;
});

test('-1.1 截断为 -1', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(-1.1, 0);
  return buf.readInt16LE(0) === -1;
});

test('-1.9 截断为 -1', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(-1.9, 0);
  return buf.readInt16LE(0) === -1;
});
...
test('Math.PI 截断', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(Math.PI, 0);
  return buf.readInt16LE(0) === 3;
});

test('Math.E 截断', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(Math.E, 0);
  return buf.readInt16LE(0) === 2;
});

test('Math.SQRT2 截断', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(Math.SQRT2, 0);
  return buf.readInt16LE(0) === 1;
});
