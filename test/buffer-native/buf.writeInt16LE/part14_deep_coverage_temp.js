...
test('写入不对齐的地址（奇数 offset）', () => {
  ...
  buf.writeInt16BE(0x9ABC, 5);
  return buf[1] === 0x12 && buf[2] === 0x34 &&
         buf[3] === 0x56 && buf[4] === 0x78 &&
         buf[5] === 0x9A && buf[6] === 0xBC;
});

test('所有可能的 2 字节对齐位置', () => {
  const buf = Buffer.alloc(20);
  for (let i = 0; i < 18; i++) {
    buf.writeInt16BE((i + 1) * 100, i);
  }
  // 验证几个关键位置
  return buf.readInt16BE(0) === 100 &&
         buf.readInt16BE(9) === 1000 &&
         buf.readInt16BE(17) === 1800;
});
...
test('连续写入相同位置覆盖', () => {
  ...
  buf.writeInt16BE(0x9ABC, 0);
  return buf[0] === 0x9A && buf[1] === 0xBC;
});
...
test('在 64KB buffer 中写入', () => {
  ...
  buf.writeInt16BE(0x9ABC, 65534);
  return buf.readInt16BE(0) === 0x1234 &&
         buf.readInt16BE(32768) === 0x5678 &&
         buf.readInt16BE(65534) === 0x9ABC;
});
