// buf.writeInt16LE() - TypedArray 和兼容性测试
...
test('Uint8Array 可以调用 writeInt16LE', () => {
  const arr = new Uint8Array(4);
  const buf = Buffer.from(arr.buffer);
  buf.writeInt16LE(1000, 0);
  return buf[0] === 0xE8 && buf[1] === 0x03;
});
...
test('共享 ArrayBuffer 的 Buffer 写入', () => {
  ...
  buf.writeInt16LE(999, 0);
  buf.writeInt16LE(-999, 2);
  return buf.readInt16LE(0) === 999 && buf.readInt16LE(2) === -999;
});
...
test('多个 Buffer 共享 ArrayBuffer', () => {
  ...
  return fullBuf.readInt16LE(0) === 100 && fullBuf.readInt16LE(4) === 200;
});
...
test('Buffer slice 后写入不影响原 buffer', () => {
  ...
  slice.writeInt16LE(888, 0);
  // slice 写入会影响 original 因为是视图
  return original.readInt16LE(2) === 888;
});

test('Buffer subarray 写入影响原 buffer', () => {
  const original = Buffer.alloc(10);
  const sub = original.subarray(3, 8);
  sub.writeInt16LE(777, 0);
  return original.readInt16LE(3) === 777;
});

// 方法链式调用
test('连续调用 writeInt16LE', () => {
  const buf = Buffer.alloc(8);
  const ret1 = buf.writeInt16LE(100, 0);
  const ret2 = buf.writeInt16LE(200, ret1);
  const ret3 = buf.writeInt16LE(300, ret2);
  const ret4 = buf.writeInt16LE(400, ret3);
  return buf.readInt16LE(0) === 100 &&
         buf.readInt16LE(2) === 200 &&
         buf.readInt16LE(4) === 300 &&
         buf.readInt16LE(6) === 400;
});

// 与其他 write 方法混合使用
test('writeInt16LE 和 writeInt8 混合', () => {
  ...
  return buf[0] === 10 &&
         buf.readInt16LE(1) === 1000 &&
         buf[3] === 20 &&
         buf.readInt16LE(4) === 2000;
});

test('writeInt16LE 和 writeUInt16LE 在同一位置', () => {
  const buf1 = Buffer.alloc(4);
  const buf2 = Buffer.alloc(4);
  buf1.writeInt16LE(-1000, 0);
  buf2.writeUInt16LE(0xFFFF - 1000 + 1, 0);
  return buf1[0] === buf2[0] && buf1[1] === buf2[1];
});

test('writeInt16LE 和 writeInt32LE 重叠', () => {
  const buf = Buffer.alloc(8);
  buf.writeInt32LE(0x12345678, 0);
  buf.writeInt16LE(-21555, 2); // 使用有效范围内的负数 (0xABCD 作为有符号数)
  return buf[0] === 0x78 && buf[1] === 0x56 &&
         buf[2] === 0xCD && buf[3] === 0xAB;
});
...
test('在大 buffer 中写入', () => {
  ...
  buf.writeInt16LE(12345, 5000);
  return buf.readInt16LE(5000) === 12345;
});

test('在最小可用 buffer (2字节) 中写入', () => {
  const buf = Buffer.alloc(2);
  buf.writeInt16LE(999, 0);
  return buf.readInt16LE(0) === 999;
});
