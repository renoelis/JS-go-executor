// buf.writeInt16LE() - 第7轮补充：内存视图和 TypedArray/ArrayBuffer 交互测试
...
test('从 ArrayBuffer 创建的 Buffer', () => {
  ...
  return buf.readInt16LE(0) === 1234;
});

test('从 Uint8Array 创建的 Buffer', () => {
  const u8 = new Uint8Array(10);
  const buf = Buffer.from(u8);
  buf.writeInt16LE(5678, 0);
  return buf.readInt16LE(0) === 5678;
});
...
test('从 Uint16Array 创建的 Buffer', () => {
  ...
  buf.writeInt16LE(9999, 0);
  return buf.readInt16LE(0) === 9999;
});

// Buffer.allocUnsafe 的行为
test('allocUnsafe 的 Buffer 可以正常写入', () => {
  const buf = Buffer.allocUnsafe(10);
  buf.writeInt16LE(111, 0);
  buf.writeInt16LE(222, 2);
  return buf.readInt16LE(0) === 111 && buf.readInt16LE(2) === 222;
});
...
test('Buffer 和底层 ArrayBuffer 共享内存', () => {
  const ab = new ArrayBuffer(10);
  const buf = Buffer.from(ab);
  buf.writeInt16LE(4660, 0); // 0x1234, LE: [0x34, 0x12]
...
test('修改 Buffer 影响同一 ArrayBuffer 的 DataView', () => {
  ...
  buf.writeInt16LE(22136, 2); // 0x5678, LE: [0x78, 0x56]
  return dv.getUint8(2) === 0x78 && dv.getUint8(3) === 0x56;
});

test('Buffer.subarray 共享底层内存', () => {
  const buf = Buffer.alloc(10);
  const sub = buf.subarray(2, 8);
  sub.writeInt16LE(-21555, 0);
  return buf.readInt16LE(2) === -21555;
});

test('Buffer.slice 创建新副本（旧行为）或共享内存（新行为）', () => {
  const buf = Buffer.alloc(10);
  const sliced = buf.slice(2, 8);
  sliced.writeInt16LE(-4351, 0);
  // 在新版 Node 中 slice 也共享内存
  return buf.readInt16LE(2) === -4351 || buf.readInt16LE(2) === 0;
});
...
test('在 subarray 的边界写入', () => {
  ...
  sub.writeInt16LE(7777, 0);
  sub.writeInt16LE(8888, 8);
  return buf.readInt16LE(5) === 7777 && buf.readInt16LE(13) === 8888;
});

test('在 subarray 中间写入不影响外部区域', () => {
  const buf = Buffer.alloc(20, 0xFF);
  const sub = buf.subarray(8, 12);
  sub.writeInt16LE(0, 0);
  return buf[7] === 0xFF && buf[8] === 0x00 && buf[9] === 0x00 && buf[10] === 0xFF;
});

// TypedArray 的字节序问题
test('Buffer 与 DataView 字节序一致性 - LE 写入 BE 读取', () => {
  const ab = new ArrayBuffer(10);
  const buf = Buffer.from(ab);
  const dv = new DataView(ab);
  buf.writeInt16LE(4386, 0); // 0x1122, LE: [0x22, 0x11]
  // DataView getInt16(offset, littleEndian=false) 默认 BE 读取：[0x22, 0x11] → 0x2211
  return dv.getInt16(0, false) === 0x2211;
});

test('Buffer LE 与 DataView LE 一致', () => {
  const ab = new ArrayBuffer(10);
  const buf = Buffer.from(ab);
  const dv = new DataView(ab);
  buf.writeInt16LE(4386, 0); // 0x1122, LE: [0x22, 0x11]
  // DataView getInt16(offset, littleEndian=true) LE 读取：[0x22, 0x11] → 0x1122
  return dv.getInt16(0, true) === 4386;
});
...
test('从 ArrayBuffer 中间创建的 Buffer', () => {
  ...
  buf.writeInt16LE(-26215, 0);
  const u8 = new Uint8Array(ab);
  return u8[5] === 0x99 && u8[6] === 0x99;
});

// 在 TypedArray 上调用 Buffer 方法
test('Uint8Array 上调用 writeInt16LE', () => {
  const u8 = new Uint8Array(10);
  try {
    Buffer.prototype.writeInt16LE.call(u8, 100, 0); // 100 = 0x0064, LE: [0x64, 0x00]
    return u8[0] === 0x64 && u8[1] === 0x00;
  } catch (e) {
    // 某些实现可能不允许
...
// 在类数组对象上调用
test('普通数组上调用 writeInt16LE 可以成功', () => {
  const arr = [0, 0, 0, 0];
  Buffer.prototype.writeInt16LE.call(arr, 100, 0); // 100 = 0x0064, LE: [0x64, 0x00]
  // 会写入到 arr[0] 和 arr[1]
  return arr[0] === 0x64 && arr[1] === 0x00;
});

test('null 作为 this 抛出错误', () => {
  try {
    Buffer.prototype.writeInt16LE.call(null, 100, 0);
    return false;
  } catch (e) {
...
test('undefined 作为 this 抛出错误', () => {
  try {
    Buffer.prototype.writeInt16LE.call(undefined, 100, 0);
    return false;
  } catch (e) {
...
test('SharedArrayBuffer 创建的 Buffer (如果支持)', () => {
  try {
    ...
    buf.writeInt16LE(6543, 0);
    return buf.readInt16LE(0) === 6543;
  } catch (e) {
...
test('Buffer.concat 后可以正常写入', () => {
  ...
  concatenated.writeInt16LE(-18, 2); // 0xFFEE 作为有符号数是 -18
  return concatenated.readInt16LE(2) === -18;
});
...
test('多层 subarray 嵌套写入', () => {
  ...
  sub2.writeInt16LE(4951, 0);
  // sub2 的 offset 0 对应 buf 的 offset 5
  return buf.readInt16LE(5) === 4951;
});
