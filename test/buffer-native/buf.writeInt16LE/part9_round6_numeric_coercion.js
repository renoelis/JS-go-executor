// buf.writeInt16LE() - 第6轮补充：数值强制转换和截断行为深度测试
...
test('小数向下截断 - 正数', () => {
  ...
  buf.writeInt16LE(3.14, 0);
  return buf.readInt16LE(0) === 3;
});

test('小数向下截断 - 接近整数', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16LE(3.9999, 0);
  return buf.readInt16LE(0) === 3;
});

test('小数向下截断 - 负数', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16LE(-2.5, 0);
  return buf.readInt16LE(0) === -2;
});

test('小数向下截断 - 负数接近整数', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16LE(-2.9999, 0);
  return buf.readInt16LE(0) === -2;
});
...
test('offset 为小数会被截断或抛出错误', () => {
  try {
    ...
    const result = buf.writeInt16LE(100, 2.5);
    // Node 可能接受并截断，或者抛出错误
    return result === 4;
  } catch (e) {
...
test('字符串后缀非数字转为 NaN 再转为 0', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16LE('100abc', 0);
  return buf.readInt16LE(0) === 0 || buf.readInt16LE(0) === 100;
});
...
test('布尔值 true 转为 1', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16LE(true, 0);
  return buf.readInt16LE(0) === 1;
});

test('布尔值 false 转为 0', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16LE(false, 0);
  return buf.readInt16LE(0) === 0;
});
...
test('BigInt 大值可能被截断或抛出错误', () => {
  try {
    const buf = Buffer.alloc(4);
    buf.writeInt16LE(BigInt(1000000), 0);
    return true;
  } catch (e) {
...
test('多元素数组根据类型转换规则', () => {
  const buf = Buffer.alloc(4);
  try {
    buf.writeInt16LE([100, 200, 300], 0);
