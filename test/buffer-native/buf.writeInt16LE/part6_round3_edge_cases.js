// buf.writeInt16LE() - 第3轮补充：参数缺省和特殊场景
...
test('缺省 offset 参数使用默认值 0', () => {
  ...
  const result = buf.writeInt16LE(1000);
  return result === 2 && buf[0] === 0xE8 && buf[1] === 0x03;
});
...
test('写入 32766 (最大值-1)', () => {
  ...
  return buf[0] === 0xFE && buf[1] === 0x7F && buf.readInt16LE(0) === 32766;
});

test('写入 -32767 (最小值+1)', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16LE(-32767, 0);
  return buf[0] === 0x01 && buf[1] === 0x80 && buf.readInt16LE(0) === -32767;
});

// 返回值测试
test('写入后返回值总是 offset + 2', () => {
  const buf = Buffer.alloc(10);
  let ret = buf.writeInt16LE(100, 0);
  if (ret !== 2) return false;
  ret = buf.writeInt16LE(200, 1);
  if (ret !== 3) return false;
  ret = buf.writeInt16LE(300, 7);
...
test('返回值可以用作下一次写入的 offset', () => {
  const buf = Buffer.alloc(10);
  let offset = 0;
  offset = buf.writeInt16LE(100, offset);
  offset = buf.writeInt16LE(200, offset);
  offset = buf.writeInt16LE(300, offset);
  return offset === 6 &&
...
test('值为科学计数法字符串', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16LE('1e3', 0);
  const expected = Buffer.alloc(4);
  expected.writeInt16LE(1000, 0);
  return buf[0] === expected[0] && buf[1] === expected[1];
});
