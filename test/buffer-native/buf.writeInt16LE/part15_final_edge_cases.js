...
test('writeInt16LE.length 应该是 1', () => {
  return Buffer.prototype.writeInt16LE.length === 1;
});

test('writeInt16LE.name 应该是 "writeInt16LE"', () => {
  return Buffer.prototype.writeInt16LE.name === 'writeInt16LE';
});

test('writeInt16LE 是一个函数', () => {
  return typeof Buffer.prototype.writeInt16LE === 'function';
});
...
test('普通对象作为 this 可以写入', () => {
  ...
  Buffer.prototype.writeInt16LE.call(obj, 100, 0); // 100 = 0x0064, LE: [0x64, 0x00]
  return obj[0] === 0x64 && obj[1] === 0x00;
});

test('带额外属性的对象作为 this', () => {
  const obj = { 0: 0, 1: 0, length: 2, extra: 'data' };
  Buffer.prototype.writeInt16LE.call(obj, 200, 0); // 200 = 0x00C8, LE: [0xC8, 0x00]
  return obj[0] === 0xC8 && obj[1] === 0x00 && obj.extra === 'data';
});
...
test('在 subarray 的开始位置写入', () => {
  ...
  sub.writeInt16LE(0x5678, 0);
  return buf[5] === 0x78 && buf[6] === 0x56;
});

test('在 subarray 的结束位置写入', () => {
  const buf = Buffer.alloc(10);
  const sub = buf.subarray(2, 8);
  sub.writeInt16LE(-21555, 4); // 0xABCD as signed, LE: [0xCD, 0xAB]
  return buf[6] === 0xCD && buf[7] === 0xAB;
});
...
test('写入质数序列', () => {
  ...
  return buf.readInt16LE(0) === 2 && buf.readInt16LE(18) === 29;
});
...
test('写入斐波那契前8项', () => {
  ...
  return buf.readInt16LE(0) === 1 && buf.readInt16LE(14) === 21;
});
...
test('写入位运算结果 - AND', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16LE(0xFF & 0x1234, 0);
  return buf[0] === 0x34 && buf[1] === 0x00;
});

test('写入位运算结果 - OR', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16LE(0x1200 | 0x0034, 0);
  return buf[0] === 0x34 && buf[1] === 0x12;
});

test('写入位运算结果 - XOR', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16LE(0xFFFF ^ 0xEDCB, 0);
  return buf.readInt16LE(0) === (0xFFFF ^ 0xEDCB);
});
...
test('连续写入后 compare 返回 0', () => {
  ...
  for (let i = 0; i < 5; i++) {
    buf1.writeInt16LE(i * 100, i * 2);
    buf2.writeInt16LE(i * 100, i * 2);
  }
...
test('写入特定模式后 indexOf 能找到', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(0x1234, 2); // LE: [0x34, 0x12]
  const needle = Buffer.from([0x34, 0x12]);
  return buf.indexOf(needle) === 2;
});

test('写入后 includes 能检测', () => {
  const buf = Buffer.alloc(10);
  buf.writeInt16LE(0x1234, 2);
...
test('在 Int16Array 视图上调用', () => {
  ...
  Buffer.prototype.writeInt16LE.call(i16, 100, 0);
  // 写入成功，i16[0] 应该包含数据
...
test('在 Uint32Array 视图上调用', () => {
  ...
  Buffer.prototype.writeInt16LE.call(u32, 100, 0);
  return true; // 成功调用即可
});
...
test('小数值正确截断为整数', () => {
  const buf = Buffer.alloc(4);
  buf.writeInt16LE(100.9, 0);
  buf.writeInt16LE(200.1, 2);
  return buf.readInt16LE(0) === 100 && buf.readInt16LE(2) === 200;
});
...
test('最大值 + 0', () => {
  ...
  return buf.readInt16LE(0) === 32767 && buf.readInt16LE(2) === 0;
});
...
test('最小值 + 最大值', () => {
  ...
  return buf.readInt16LE(0) === -32768 && buf.readInt16LE(2) === 32767;
});
