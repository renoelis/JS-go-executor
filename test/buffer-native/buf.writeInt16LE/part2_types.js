...
test('浮点数会被截断为整数', () => {
  ...
  buf.writeInt16LE(1234.56, 0);
  const expected = Buffer.alloc(4);
  expected.writeInt16LE(1234, 0);
...
test('超出范围的正数会抛出错误', () => {
  try {
    const buf = Buffer.alloc(4);
    buf.writeInt16LE(32768, 0); // 超出最大值
    return false;
  } catch (e) {
...
test('超出范围的负数会抛出错误', () => {
  try {
    const buf = Buffer.alloc(4);
    buf.writeInt16LE(-32769, 0); // 超出最小值
    return false;
  } catch (e) {
    return e.message.includes('out of range');
  }
