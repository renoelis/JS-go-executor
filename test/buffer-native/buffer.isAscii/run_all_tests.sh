...
if [ "$total_tests" -gt 0 ]; then
  ...
  if [ "$total_failed" -eq 0 ]; then
    echo ""
    echo "🎉 所有测试通过！buffer.isAscii API 与 Node.js v25.0.0 完全兼容！"
    exit 0
  else
    echo ""
