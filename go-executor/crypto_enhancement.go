...
func (ce *CryptoEnhancer) SetupCryptoEnvironment(runtime *goja.Runtime) error {
	...
	// 同时设置到 global 对象（如果存在的话）
	if globalVal := runtime.Get("global"); globalVal != nil && !goja.IsUndefined(globalVal) {
		if globalObj, ok := globalVal.(*goja.Object); ok {
...
func (ce *CryptoEnhancer) loadCryptoJS(runtime *goja.Runtime) error {
	...
	if err != nil {
		return fmt.Errorf("获取crypto-js代码失败: %w", err)
	}

	// 执行crypto-js代码并确保设置全局对象
	...
	if err != nil {
		return fmt.Errorf("执行crypto-js代码失败: %w", err)
	}

	// 检查加载结果
	if result != nil && result.ToBoolean() {
		// fmt.Printf("✅ CryptoJS 全局对象设置成功\n")
	} else {
		return fmt.Errorf("crypto-js 加载后 CryptoJS 全局对象设置失败")
	}
...
func (ce *CryptoEnhancer) getCryptoJSCode() (string, error) {
	...
	if embeddedCryptoJS != "" {
		cryptoJSContent = embeddedCryptoJS
		loadSource = "嵌入式文件"
...
func (ce *CryptoEnhancer) enhanceWithNativeAPIs(runtime *goja.Runtime) error {
	...
	// 第一步：桥接 crypto-js 的功能到 crypto 对象
	if err := ce.bridgeCryptoJS(runtime, cryptoObj); err != nil {
		fmt.Printf("⚠️  桥接 crypto-js 失败: %v\n", err)
		// 不返回错误，继续使用Go原生实现
	}

	// 第二步：添加Go原生实现的方法（补齐crypto-js没有的）
...
// bridgeCryptoJS 桥接CryptoJS功能到crypto对象
func (ce *CryptoEnhancer) bridgeCryptoJS(runtime *goja.Runtime, cryptoObj *goja.Object) error {
	// 检查 CryptoJS 是否存在
	cryptoJSVal := runtime.Get("CryptoJS")
	if cryptoJSVal == nil || goja.IsUndefined(cryptoJSVal) {
		return fmt.Errorf("CryptoJS 全局对象不存在")
	}

	// fmt.Printf("✅ 检测到 CryptoJS 全局对象\n")

	// 创建桥接代码，将 CryptoJS 的功能映射到 crypto 对象
	bridgeCode := `
		(function(crypto, CryptoJS) {
			...
			crypto.createCipheriv = function(algorithm, key, iv) {
				...
				return {
					...
					update: function(data, inputEncoding, outputEncoding) {
						this._data += data;
						return ''; // crypto-js 不支持流式，在 final 中处理
					},
					final: function(outputEncoding) {
						outputEncoding = outputEncoding || 'hex';
						var encrypted = CryptoJS.AES.encrypt(this._data, keyWords, {
							iv: ivWords,
				...
				return {
					...
					final: function(outputEncoding) {
						...
						var cipherParams = CryptoJS.lib.CipherParams.create({
							ciphertext: CryptoJS.enc.Hex.parse(this._data)
						});

						var decrypted = CryptoJS.AES.decrypt(cipherParams, keyWords, {
							iv: ivWords,
							mode: CryptoJS.mode.CBC,
							padding: CryptoJS.pad.Pkcs7
						});
			...
			// 映射 createHash (如果Go原生没有提供)
			if (!crypto.createHash) {
				crypto.createHash = function(algorithm) {
					var hasher;
					algorithm = algorithm.toLowerCase();

					if (algorithm === 'md5') hasher = CryptoJS.algo.MD5.create();
					else if (algorithm === 'sha1') hasher = CryptoJS.algo.SHA1.create();
					else if (algorithm === 'sha256') hasher = CryptoJS.algo.SHA256.create();
					else if (algorithm === 'sha512') hasher = CryptoJS.algo.SHA512.create();
					...
					return {
						...
						digest: function(encoding) {
							...
							else if (encoding === 'base64') return hash.toString(CryptoJS.enc.Base64);
			...
			if (!crypto.createHmac) {
				crypto.createHmac = function(algorithm, key) {
					...
					if (algorithm === 'md5') hmacAlgo = CryptoJS.HmacMD5;
					else if (algorithm === 'sha1') hmacAlgo = CryptoJS.HmacSHA1;
					else if (algorithm === 'sha256') hmacAlgo = CryptoJS.HmacSHA256;
					else if (algorithm === 'sha512') hmacAlgo = CryptoJS.HmacSHA512;
					else throw new Error('Unsupported HMAC algorithm: ' + algorithm);
			...
			// 暴露 CryptoJS 让用户可以直接访问
			crypto.CryptoJS = CryptoJS;

			// console.log('✅ crypto-js 桥接完成'); // 移除console调用，避免在某些环境中未定义
			return crypto;
		})(crypto, CryptoJS);
	`
	...
	// 执行桥接代码
	result, err := runtime.RunString(bridgeCode)
	if err != nil {
		return fmt.Errorf("桥接代码执行失败: %w", err)
	}

	// fmt.Printf("✅ crypto-js 桥接代码执行成功\n")

	// 更新 cryptoObj 为桥接后的对象
	if resultObj, ok := result.(*goja.Object); ok {
		// 复制所有属性到原始 cryptoObj
		for _, key := range resultObj.Keys() {
			cryptoObj.Set(key, resultObj.Get(key))
		}
	}
