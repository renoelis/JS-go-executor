...
function testAdvancedBufferFeatures() {
    try {
        ...
        buf2.writeInt32LE(-0x12345678, 4);  // 修正：使用合法范围内的值
        buf2.writeUInt32BE(0xFFFFFFFF, 8);
        buf2.writeUInt32LE(0x80000000, 12);

        results.int32BE = buf2.readInt32BE(0);     // 应该是 0x12345678 (305419896)
        results.int32LE = buf2.readInt32LE(4);     // 应该是 -0x12345678 (-305419896)
        results.uint32BE = buf2.readUInt32BE(8);   // 应该是 0xFFFFFFFF (4294967295)
        results.uint32LE = buf2.readUInt32LE(12);  // 应该是 0x80000000 (2147483648)
...
const result = testAdvancedBufferFeatures();
console.log(result);
return result;
